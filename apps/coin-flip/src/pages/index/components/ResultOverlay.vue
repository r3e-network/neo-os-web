  ...
  <div v-if="visible" class="cinematic-overlay" @click="$emit('close')">
    ...
    <div class="overlay-content" :class="{ 'is-win': Number(winAmount) > 0 }" aria-live="polite" role="status">
      <img
        ...
        :alt="t('winnerTitle')"
      />
      <div v-else class="congrats-text">{{ t("overlayUnlucky") }}</div>
      ...
      <div class="win-label">{{ Number(winAmount) > 0 ? t("overlayWinLabel") : t("overlayLoseLabel") }}</div>

      <div class="tap-hint">{{ t("overlayTapContinue") }}</div>
    </div>
