<template>
  <NeoCard variant="erobo-bitcoin" :class="['last-survivor-card', dangerLevel]">
    <div class="clock-header">
      <span class="clock-label text-glass-glow">{{ t("timeUntilEvent") }}</span>
      <div :class="['danger-badge-glass', dangerLevel]">
        <span class="danger-text">{{ dangerLevelText }}</span>
      </div>
    </div>

    <div class="clock-display-glass" aria-live="polite" role="timer" :aria-label="t('timeUntilEvent')">
      <span :class="['clock-time-glass', dangerLevel, { pulse: shouldPulse }]">{{ countdown }}</span>
    </div>

    <!-- Danger Level Meter -->
    <div class="danger-meter-glass">
      <div class="meter-labels">
        <span class="meter-label text-glass">{{ t("safe") }}</span>
        <span class="meter-label text-glass">{{ t("critical") }}</span>
      </div>
      <div class="meter-bar-glass">
        <div :class="['meter-fill-glass', dangerLevel]" :style="{ width: dangerProgress + '%' }"></div>
        <div class="meter-indicator-glass" :style="{ left: dangerProgress + '%' }"></div>
      </div>
    </div>

    <!-- Event Description -->
    <div class="event-description-glass">
      <span class="event-title-glass">{{ t("nextEvent") }}</span>
      <span class="event-text-glass">{{ currentEventDescription }}</span>
    </div>
  </NeoCard>
</template>
