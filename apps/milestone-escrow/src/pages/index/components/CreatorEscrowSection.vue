<template>
  <div>
    <div class="section-header">
      <span class="section-label">{{ t("createdByYou") }}</span>
    ...
    <div v-for="escrow in escrows" :key="`creator-${escrow.id}`" class="escrow-card">
      <div class="escrow-card__header">
        <div>
          <span class="escrow-title">{{ escrow.title || `#${escrow.id}` }}</span>
          <span class="escrow-subtitle">{{ formatAddressFunc(escrow.beneficiary) }}</span>
        </div>
        <StatusBadge
      ...
      <div class="escrow-metrics">
        <div>
          <span class="metric-label">{{ t("totalAmount") }}</span>
          <span class="metric-value"
            >{{ formatAmountFunc(escrow.assetSymbol, escrow.totalAmount) }} {{ escrow.assetSymbol }}</span
          >
        </div>
        <div>
          <span class="metric-label">{{ t("claimed") }}</span>
          <span class="metric-value"
            >{{ formatAmountFunc(escrow.assetSymbol, escrow.releasedAmount) }} {{ escrow.assetSymbol }}</span
          >
        </div>
      </div>
