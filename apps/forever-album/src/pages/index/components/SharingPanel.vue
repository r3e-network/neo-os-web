<template>
  <ActionModal :visible="visible" :title="t('sharePhoto')" @close="$emit('close')">
    <div class="share-body">
      <div class="share-preview" v-if="photo">
        <img :src="photo.data" mode="aspectFill" class="share-thumb" :alt="t('sharePreviewImage')" />
        <span class="share-label">{{ t("sharePreview") }}</span>
      </div>

      <div class="share-options">
        <span class="options-title">{{ t("shareOptions") }}</span>
        <div class="option-list" role="radiogroup" :aria-label="t('shareOptions')">
          <div
            class="option-item"
            role="radio"
            tabindex="0"
      ...
      <div v-if="shareMethod === 'address'" class="address-input">
        <NeoInput v-model="recipientAddress" :placeholder="t('recipientAddress')" />
      </div>

      <div class="share-note">
        <span class="note-text">{{ t("shareNote") }}</span>
      </div>
    </div>
