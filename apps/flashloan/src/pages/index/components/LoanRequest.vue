<template>
  <div class="loan-request">
    ...
    <NeoCard variant="warning" class="mb-4 text-center">
      <span class="text-glass-glow block font-bold">{{ t("instructionMode") }}</span>
      <span class="text-glass text-xs opacity-80">{{ t("instructionNote") }}</span>
    </NeoCard>

    <NeoCard v-if="status" :variant="status.type === 'error' ? 'danger' : 'erobo-neo'" class="mb-4 text-center">
      <span class="text-glass font-bold">{{ status.msg }}</span>
    </NeoCard>
    ...
    <LoanRequestForm
      ...
      @update:loanId="$emit('update:loanId', $event)"
      @lookup="$emit('lookup')"
