<template>
  <div class="tab-content">
    ...
    <div v-else class="market-grid">
      ...
      <div v-else-if="marketMachines.length === 0" class="empty-state">
        {{ t("noMachines") }}
      </div>
      <div v-else>
        <NeoCard v-if="recommendedMachines.length" variant="erobo" class="section-card">
          <span class="section-title">{{ t("recommended") }}</span>
        ...
        <div class="ranking-grid">
          <NeoCard variant="erobo" class="section-card">
            <span class="section-title">{{ t("topPlays") }}</span>
            <div class="rank-list">
              <div v-for="machine in topByPlays" :key="machine.id" class="rank-row">
                <span class="rank-name">{{ machine.name }}</span>
                <span class="rank-value">{{ machine.plays }}</span>
              </div>
            </div>
          </NeoCard>
          <NeoCard variant="erobo" class="section-card">
            <span class="section-title">{{ t("topRevenue") }}</span>
            <div class="rank-list">
        ...
        <NeoCard v-if="forSaleMachines.length" variant="erobo" class="section-card">
          <span class="section-title">{{ t("forSale") }}</span>
          <div class="grid-container">
            <GachaCard
        ...
        <NeoCard variant="erobo" class="section-card">
          <span class="section-title">{{ t("allMachines") }}</span>
          <div class="grid-container">
            <GachaCard
