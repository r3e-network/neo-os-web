...
function t(key: string, params?: Record<string, string | number>) {
  const messages: Record<string, string> = {
    ...
    agentTopologyTitle: "21-agent trust topology",
    agentTopologySubtitle: "Every trust route visible at once",
    topologyCount: "{count} routes",
    topologyNetworkLabel: "Selectable trust topology",
    topologyLegendLabel: "Topology legend",
    sourceLegend: "Source",
    targetLegend: "Target",
    activeRouteLegend: "Trusted route",
    inactiveRouteLegend: "Inactive",
    anchorHubTitle: "TrustAnchor assurance hub",
    routePlannerTitle: "Trust route planner",
...
describe("trustanchor-admin PlayArea", () => {
  ...
  it("renders the complete 21-agent topology instead of truncating the roster", () => {
    const agentAccounts = Array.from({ length: 21 }, (_, index) => ({
      agentId: index + 1,
      label: `Trust Agent ${String(index + 1).padStart(2, "0")}`,
      accountAddress: `Ntrust${String(index + 1).padStart(27, "0")}`,
      candidateTarget: `03${"b".repeat(64)}`,
      neoBalance: index + 1,
      active: true,
    }));
  ...
  it("keeps the admin surface foreground-led and free of global backdrop patches", () => {
    ...
    const theme = readFileSync(`${process.cwd()}/../trustanchor-admin/src/PlayArea.scss`, "utf8");
    ...
    expect(theme).toContain('--admin-canvas: #f7fbff;');
