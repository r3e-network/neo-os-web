...
function state(overrides: Record<string, unknown> = {}): ObservableState {
  const values = {
    ...
    readStatus: "ready",
    stats: {
      mode: "1", totalStaked: "40", totalStakers: "3", rewardPerNeo: "0",
      rewardReserve: "250000000", agentCount: "21", selectedAgentId: "2", paused: false,
    },
    user: { walletHash: `0x${"11".repeat(20)}`, stake: "8", pendingRewards: "150000000", neoCredit: "0" },
...
describe("trustanchor production PlayArea integration", () => {
  it("sends the selected whole-NEO plan through the single primary stake action", () => {
    const dispatch = vi.fn().mockResolvedValue(undefined);
    const { container } = render(<PlayArea t={t} state={state()} dispatch={dispatch} />);
    const preset = Array.from(container.querySelectorAll<HTMLButtonElement>(".trust-amount-deck button"))
  ...
  it("keeps redeem and claim in the secondary drawer", () => {
    ...
    fireEvent.click(container.querySelector(".trust-secondary-action") as HTMLButtonElement);
    expect(dispatch).toHaveBeenCalledWith("withdrawNeo", { amount: "1" });
    fireEvent.click(container.querySelectorAll<HTMLButtonElement>(".trust-drawer__nav button")[1]);
    fireEvent.click(container.querySelector(".trust-secondary-action") as HTMLButtonElement);
