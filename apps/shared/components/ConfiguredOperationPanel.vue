<template>
  <div class="configured-operation-panel">
    <div class="panel-header">
      <span class="panel-title">{{ panelTitle }}</span>
      <span v-if="panelDescription" class="panel-description">{{ panelDescription }}</span>
    </div>

    <div v-if="operation.fields.length > 0" class="panel-fields">
      <div v-for="field in operation.fields" :key="field.key" class="field-row">
    ...
    <div v-if="operation.summaryKeys?.length" class="summary-list">
      <div v-for="summary in operation.summaryKeys" :key="`${summary.valueKey}-${summary.label || summary.labelKey || ''}`" class="summary-item">
        <span class="summary-label">{{ summaryLabel(summary) }}</span>
        <span class="summary-value">{{ summaryValue(summary.valueKey) }}</span>
      </div>
    </div>
    ...
    <span v-if="statusMessage" class="panel-status">{{ statusMessage }}</span>
  </div>
</template>
