...
export function MiniAppPage({
  ...
  const brandIcon = useMemo(() => config.tabs[0]?.icon ?? DEFAULT_BRAND_ICON, [config.tabs]);
  const defaultTabKey = useMemo(() => config.tabs.find((tab) => tab.default)?.key ?? config.tabs[0]?.key ?? "", [config.tabs]);
  ...
  const infoTabs = useMemo(() => {
    ...
    if (sidebarItems.length > 0 || renderTabStats) tabs.push({ key: "stats", labelKey: "stats" });
    if (renderTabHistory) tabs.push({ key: "history", labelKey: "history" });
    config.tabs
      .filter((tab) => !tab.default && tab.key !== "stats" && tab.key !== "history" && tab.key !== "docs")
      .forEach((tab) => {
    ...
    return tabs;
  }, [sidebarItems.length, renderTabStats, renderTabHistory, config.tabs, customTabRenderers]);
  ...
  const docFeatureCount = (config as MiniAppTemplateConfig & { docFeatureCount?: number }).docFeatureCount ?? 3;
  const docFeatures = useMemo(() => {
    ...
    for (let i = 1; i <= docFeatureCount; i++) {
      ...
      if (featureName && featureName !== `feature${i}Name`) {
        features.push({ name: featureName, desc: desc !== `feature${i}Desc` ? desc : "" });
      }
    }
    return features;
  }, [t, docFeatureCount]);
  ...
  return (
    <div className={`miniapp-page theme-${name}`}>
          ...
          {sidebarItems.length > 0 && (
            <div className="sidebar-stats">
              ...
              {sidebarItems.map((item, i) => (
                <div key={`stat-${i}`} className="stat-row" style={{ "--i": i } as React.CSSProperties}>
                  <span className="stat-label">{item.label}</span>
                  <span className="stat-value">{item.value ?? t("notAvailable")}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {hasInfoContent && (
            <section className="section-info">
              ...
              <div className="info-content">
                {activeInfoTab === "stats" && (
                  <div id="tab-panel-stats" role="tabpanel" aria-labelledby="tab-stats">
                      ...
                      sidebarItems.length > 0 && (
                        <div className="auto-stats-grid">
                          {sidebarItems.map((item, i) => (
                            ...
                            >
                              <span className="auto-stat-value">{item.value ?? t("notAvailable")}</span>
                              <span className="auto-stat-label">{item.label}</span>
                            </div>
                          ))}
                        </div>
                      )
                ...
                {activeInfoTab === "history" && (
                  <div id="tab-panel-history" role="tabpanel" aria-labelledby="tab-history">
                    {renderTabHistory?.()}
                  </div>
                )}
                {customInfoTabs.map((tab) => (
                  activeInfoTab === tab.key ? (
                    <div key={tab.key} id={`tab-panel-${tab.key}`} role="tabpanel" aria-labelledby={`tab-${tab.key}`}>
            ...
            {renderComments ? renderComments() : (
              <>
                ...
                <div className="comments-input">
                  <label htmlFor="comment-input" className="sr-only">{t("commentPlaceholder")}</label>
                  <input
                    id="comment-input"
          ...
          <section className="section-docs">
            {renderDocs ? renderDocs() : (
              <details className="docs-accordion">
                <summary className="docs-title">{t("docSubtitle") || t("subtitle")}</summary>
                <div className="docs-content">
                  <p className="docs-description">{t("docDescription") || ""}</p>
                  {docSteps.length > 0 && (
