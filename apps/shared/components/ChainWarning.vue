<template>
  <div v-if="showWarning" class="chain-warning mb-4 px-4" role="alert" aria-live="assertive">
    <NeoCard variant="danger">
      <div class="flex flex-col items-center gap-2 py-1">
        <span class="text-center font-bold text-red-400">{{ title }}</span>
        <span v-if="message" class="text-center text-xs text-white opacity-80">{{ message }}</span>
