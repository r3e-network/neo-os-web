...
export function MiniAppOperationPanel({
  ...
  const getFieldValue = useCallback((opKey: string, fieldKey: string): unknown => {
    return formDataRef.current[opKey]?.[fieldKey] ?? "";
  }, []);

  const setFieldValue = useCallback((opKey: string, fieldKey: string, value: unknown) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const getFieldError = useCallback((opKey: string, fieldKey: string): string | undefined => {
  ...
  const resolveTitle = useCallback((op: OperationDefinition): string => {
    if (op.titleKey) return t(op.titleKey);
    return t(op.key);
  }, [t]);

  const resolveDescription = useCallback((op: OperationDefinition): string | undefined => {
    if (!op.descriptionKey) return undefined;
    const desc = t(op.descriptionKey);
    return desc !== op.descriptionKey ? desc : undefined;
  }, [t]);

  const resolveFieldLabel = useCallback((field: OperationFieldDefinition): string | undefined => {
    if (!field.labelKey) return undefined;
    const label = t(field.labelKey);
    return label !== field.labelKey ? label : field.labelKey;
  }, [t]);

  const resolveActionLabel = useCallback((op: OperationDefinition): string | undefined => {
    if (op.actionKey) {
  ...
  const getOperationStats = useCallback((_op: OperationDefinition): Array<{ key: string; label: string; value: string }> => {
    return [];
  }, []);
  ...
  const validateField = useCallback((opKey: string, field: OperationFieldDefinition): boolean => {
  ...
  const isFormValid = useCallback((opKey: string): boolean => {
  ...
  const validateOperation = useCallback((op: OperationDefinition): boolean => {
    if (!op.fields?.length) return true;
  ...
  const isSubmitting = useCallback((opKey: string): boolean => {
    return submittingOps.has(opKey);
  }, [submittingOps]);

  const submitOperation = useCallback(async (op: OperationDefinition) => {
  ...
  return (
    <div className="operation-panels">
      {operations.map((operation) => (
        <div key={operation.key} className="neo-card neo-card--erobo operation-card">
          ...
          <div className="neo-card__body">
            ...
            {resolveDescription(operation) && (
              <p className="operation-description">{resolveDescription(operation)}</p>
            )}
          ...
          <div className="field-toggle">
            <label className="toggle-label">
              <input
                ...
                aria-label={label || field.key}
                onChange={(e) => setFieldValue(opKey, field.key, e.target.checked)}
              />
  ...
  function renderInput(
    ...
    return (
      <div className={`neo-input${config.error ? " neo-input--error" : ""}`}>
        ...
        {config.error && (
          <span id={`${inputId}-error`} className="neo-input__error" role="alert" aria-live="assertive">
            {config.error}
          </span>
        )}
      </div>
