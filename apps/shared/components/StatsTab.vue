<template>
  <div class="stats-tab" :aria-label="ariaLabel || t('statistics')">
    ...
    <div v-if="rowItems.length > 0" class="stats-tab__rows">
      <div class="stats-tab__card">
        <span v-if="rowsTitle" class="stats-tab__rows-title">{{ rowsTitle }}</span>
        <StatsDisplay :items="rowItems" layout="rows" :loading="loading" />
      </div>
    </div>

    <!-- Extra content slot -->
    <div v-if="$slots.default" class="stats-tab__extra">
      <slot />
    </div>
  </div>
</template>
