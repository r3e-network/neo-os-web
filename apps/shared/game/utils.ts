...
/**
 * Convert fixed8 bigint/number/string to a display string like "1.23".
 * Delegates to the shared `fromFixed8` + `toFixed(maxDecimals)` pattern.
 */
export function gasFromFixed8(fixed8: unknown, maxDecimals = 2): number {
...
/**
 * Default Solved-event slot layout shared by most reward games.
 * Games with a different layout pass their own `slots` override.
 */
 ...
 *
 * Strategy: take the MAX totalWon per player (robust to any indexer sort
 * direction) and rank by descending totalWon.  Also extracts the caller's
 * own history rows from the same event scan.
 *
 * @param events      Raw event array from the chain indexer.
 * @param playerHash  Script-hash of the connected wallet (or "" for anonymous).
 * @param slots       Optional override of slot indices in the event state.
 * @returns           `{ ranked, mine }` — the global leaderboard and the
 *                    player's own history rows (newest-first, up to 20).
 */
