<template>
  <div class="streak-section">
    <div :class="['streak-fire-ring', streakTier, { active: currentStreak > 0 }]">
      <!-- Flame icon that grows with streak -->
      <div class="flame-stack" aria-hidden="true">
        <span class="flame flame-core">{{ flameEmoji }}</span>
        <span v-if="currentStreak >= 7" class="flame flame-outer">{{ flameEmoji }}</span>
        <span v-if="currentStreak >= 30" class="flame flame-inferno">{{ flameEmoji }}</span>
      </div>
      <span class="streak-number">{{ currentStreak }}</span>
    </div>
    <span class="streak-text">{{ t("dayStreak") }}</span>
    <span class="streak-tier-badge" :class="streakTier">{{ streakTierLabel }}</span>
...
const streakTier = computed(() => {
  if (props.currentStreak >= 30) return "inferno";
  if (props.currentStreak >= 7) return "blaze";
  if (props.currentStreak > 0) return "spark";
  return "cold";
});

const streakTierLabel = computed(() => {
  if (props.currentStreak >= 30) return "INFERNO";
  if (props.currentStreak >= 7) return "BLAZE";
  if (props.currentStreak > 0) return "SPARK";
  return "";
...
.streak-fire-ring {
  ...
  &.inferno {
    ...
    animation: inferno-pulse 1.5s ease-in-out infinite alternate;
  }
}
...
.flame {
  ...
  &.flame-outer {
    ...
    left: -10px;
    top: 2px;
    animation: flame-dance 1.2s ease-in-out infinite alternate-reverse;
    opacity: 0.8;
  }
  &.flame-inferno {
    font-size: 14px;
...
.streak-number {
  ...
  .inferno & {
    font-size: 36px;
    background: linear-gradient(180deg, #fff 0%, #fbbf24 50%, #ef4444 100%);
    -webkit-background-clip: text;
    background-clip: text;
...
@media (max-width: 480px) {
  .streak-fire-ring {
    width: 72px;
    height: 72px;
    &.blaze { width: 80px; height: 80px; }
    &.inferno { width: 88px; height: 88px; }
  }
  .streak-number {
    font-size: 22px;
    .blaze & { font-size: 26px; }
    .inferno & { font-size: 28px; }
  }
}

@media (prefers-reduced-motion: reduce) {
  .streak-fire-ring, .flame, .streak-tier-badge {
    animation: none;
    transition: none;
  }
}
