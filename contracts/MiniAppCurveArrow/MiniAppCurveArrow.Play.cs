...
{
    public partial class MiniAppCurveArrow
    {
        #region StartGame (pay entry, reserve reward, start the solve clock)
        /// <summary>
        /// Start a Curve Arrow run. Consumes the per-difficulty entry from the player's
        /// prepaid credit into the reward pool, reserves the full base reward from the
        /// free pool, sets the deadline and marks the game active. The level layouts are
        /// dealt inside the TEE session service; only the finalize result (verified by
        /// the kernel) settles the game. Returns the gameId.
        /// </summary>
