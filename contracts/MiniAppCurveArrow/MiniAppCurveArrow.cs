...
{
    /// <summary>
    /// MiniAppCurveArrow — curving-arrow archery settled through the generic Morpheus
    /// kernel: the level deal is sealed inside the TEE session service, only a 32-byte
    /// commitment hash ever reaches the chain, and wins are paid against the
    /// kernel-verified finalize result (the kernel's shared RUNTIME_VERIFIER validates
    /// the TEE fulfillment, so the game trusts caller==oracle rather than a per-game
    /// signer).
    ///
    /// MODEL (GAS only, BASE UNITS, 1 GAS = 1e8): the player prepays entry credit
    /// (deposit-then-act, memo "miniapp-curve-arrow:entry"); startGame consumes the entry
    /// into the reward pool, reserves the fixed base reward (0.1 / 0.5 / 1 GAS by
    /// difficulty), sets the deadline and marks the game active. The player steers the
    /// arrow through the interactive enclave session off-chain, then calls
    /// finalizeGame(gameId, sealed op-log) which submits ONE kernel request; the kernel
    /// replays the run, signs the result and delivers it to onMiniAppResult, which
    /// credits the FULL base reward as a PULL-PAYMENT claim (withdraw()) once the
    /// attested cleared-level count reaches the difficulty target (3 / 5 / 7). This
    /// game has no undos: MAX_UNDOS is 0 and any nonzero undo count is rejected.
    ///
    ...
    /// </summary>
    [DisplayName("MiniAppCurveArrow")]
    [ManifestExtra("Author", "R3E Network")]
    [ManifestExtra("Email", "dev@r3e.network")]
    [ManifestExtra("Version", "3.0.0")]
    [ManifestExtra("Description", "Curving-arrow archery settled through the generic Morpheus oracle kernel: run sealed in the TEE, kernel-verified finalize result, target-levels payout, pull-payment winnings.")]
    ...
    public partial class MiniAppCurveArrow : SmartContract
    {
        ...
        private const int MAX_UNDOS = 0;              // this game has no undo mechanic
        private const int UNDO_PENALTY_PCT = 0;       // no undos, no penalty
        ...
        {
            if (d == 0) return 180_000;
            if (d == 1) return 300_000;
            return 600_000;
        }
        ...
        public struct Game
        {
            ...
            public BigInteger Undos;       // TEE-tracked; recorded from the finalize result (always 0 here)
            ...
            public BigInteger Score;       // TEE-attested levels cleared from the finalize result
        }
        ...
        // slots 1 (player) and 5 (totalWon) and reads the payout from slot 4. Do not reorder.
        [DisplayName("Solved")]
        public static event Action<BigInteger, UInt160, BigInteger, BigInteger, BigInteger, BigInteger> OnSolved; // gameId, player, difficulty, elapsedMs, payout, totalWon
