...
namespace NeoMiniAppPlatform.Contracts
{
    ...
    {
        #region StartGame (pay entry, reserve reward, await the TEE bind)
        /// <summary>
        /// Start a Sheep Solitaire challenge. Consumes the per-difficulty entry from the player's
        /// prepaid credit into the reward pool and reserves the full base reward from the
        /// free pool. The puzzle is NOT known here — it is derived inside the TEE, whose
        /// bind signature fixes the commitment via bindPuzzle. Returns the gameId.
        /// </summary>
        ...
        #region ExpireGame (release reservation / refund unbound entry) — PERMISSIONLESS
        /// <summary>
        /// Close a game that can no longer settle. Bound + past deadline AND the
        /// settlement grace window: release the reward reservation (Status 3).
        /// Started but never bound for over DEAL_TTL_MS: additionally refund the
        /// entry to the player's credit (Status 4). Returns the new status.
        /// </summary>
