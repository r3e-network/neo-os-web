...
namespace NeoMiniAppPlatform.Contracts.Tests
{
    ...
    /// undo penalty ladder.
    /// </summary>
    public class MiniAppFlappyDashIntegrationTests
    {
        ...
        private const long LIMIT0_MS = 120_000;
        private const long MIN_SOLVE0_MS = 20_000;
        private const long MIN_SOLVE1_MS = 30_000;
        ...
        private static BigInteger Play(TestEngine engine, FlappyDashContract flappy, GameOracleMockFixtureContract oracle,
            UInt160 player, int difficulty, long elapsedMs, int undos)
        {
            engine.SetTransactionSigners(player);
            BigInteger gameId = flappy.startGame(player, difficulty)!.Value;
            ...
            return I(flappy.getGame(gameId)!, "payout");
        }
        ...
        public void CompleteFlow_EasyDifficulty()
        {
            ...
            BigInteger payout = Play(engine, flappy, oracle, player, 0, MIN_SOLVE0_MS + 5000, 0);
        ...
        public void CompleteFlow_MediumDifficulty()
        {
            ...
            BigInteger payout = Play(engine, flappy, oracle, player, 1, MIN_SOLVE1_MS + 5000, 0);
            Assert.Equal(new BigInteger(REWARD1), payout);
            Assert.Equal(new BigInteger(REWARD1), flappy.creditOf(player));
            AssertSolvent(engine, flappy, funder, player);
        }
        ...
        {
            var (engine, flappy, oracle, funder, player) = Setup(10);
            DepositEntry(engine, flappy, player, ENTRY2);
            BigInteger payout = Play(engine, flappy, oracle, player, 2, MIN_SOLVE2_MS + 5000, 0);
            Assert.Equal(new BigInteger(REWARD2), payout);
            Assert.Equal(new BigInteger(REWARD2), flappy.creditOf(player));
            AssertSolvent(engine, flappy, funder, player);
        }
        ...
        public void CompleteFlow_UndoLadderPays70_40_10Percent()
        {
            ...
            Assert.Equal(new BigInteger(REWARD0 * 70 / 100), Play(engine, flappy, oracle, player, 0, MIN_SOLVE0_MS + 5000, 1));
            Assert.Equal(new BigInteger(REWARD0 * 40 / 100), Play(engine, flappy, oracle, player, 0, MIN_SOLVE0_MS + 5000, 2));
            Assert.Equal(new BigInteger(REWARD0 * 10 / 100), Play(engine, flappy, oracle, player, 0, MIN_SOLVE0_MS + 5000, 3));
            AssertSolvent(engine, flappy, funder, player);
        }
        ...
        public void AllDifficulties_PayCorrectRewards()
        {
            ...
            DepositEntry(engine, flappy, player, ENTRY0 + ENTRY1 + ENTRY2);
            Assert.Equal(new BigInteger(REWARD0), Play(engine, flappy, oracle, player, 0, MIN_SOLVE0_MS + 5000, 0));
            Assert.Equal(new BigInteger(REWARD1), Play(engine, flappy, oracle, player, 1, MIN_SOLVE1_MS + 5000, 0));
            Assert.Equal(new BigInteger(REWARD2), Play(engine, flappy, oracle, player, 2, MIN_SOLVE2_MS + 5000, 0));
            ...
            engine.SetTransactionSigners(player);
            BigInteger totalCredit = flappy.creditOf(player) ?? 0;
            BigInteger before = engine.Native.GAS.BalanceOf(player) ?? 0;
            flappy.withdraw(player);
