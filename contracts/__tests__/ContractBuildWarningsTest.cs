...
namespace NeoMiniAppPlatform.Contracts.Tests
{
    public class ContractBuildWarningsTest
    {
        ...
        [Fact]
        public void OracleServiceBuildHasNoNullableWarnings()
        {
            string repoRoot = ContractSourceAssertions.FindRepoRoot();
            string projectPath = Path.Combine(repoRoot, "contracts", "OracleService", "OracleService.csproj");
            ...
            List<string> warnings = output
                .Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)
                .Where(line => line.Contains("warning CS", StringComparison.Ordinal) &&
                               line.Contains("contracts/OracleService/", StringComparison.Ordinal))
                .ToList();

            Assert.True(
                warnings.Count == 0,
                "Expected no OracleService compiler warnings, but found:" + Environment.NewLine + string.Join(Environment.NewLine, warnings));
        }

        [Fact]
        public void MiniAppDiceGameBuildHasNoBetLimitsWarnings()
        {
            string repoRoot = ContractSourceAssertions.FindRepoRoot();
            string projectPath = Path.Combine(repoRoot, "contracts", "MiniAppDiceGame", "MiniAppDiceGame.csproj");
            ...
            List<string> warnings = output
                .Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)
                .Where(line => line.Contains("warning CS", StringComparison.Ordinal) &&
                               line.Contains("contracts/MiniAppBase/MiniAppBase.BetLimits.cs", StringComparison.Ordinal))
                .ToList();

            Assert.True(
                warnings.Count == 0,
                "Expected no MiniAppBase.BetLimits compiler warnings, but found:" + Environment.NewLine + string.Join(Environment.NewLine, warnings));
        }
    }
}
