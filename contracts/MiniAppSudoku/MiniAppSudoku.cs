...
{
    /// <summary>
    /// MiniAppSudoku — timed Sudoku settled through the generic Morpheus kernel: the
    ...
    /// (deposit-then-act, memo "miniapp-sudoku:entry"); startGame consumes the entry
    ...
    /// </summary>
    [DisplayName("MiniAppSudoku")]
    [ManifestExtra("Author", "R3E Network")]
    [ManifestExtra("Email", "dev@r3e.network")]
    [ManifestExtra("Version", "3.0.0")]
    [ManifestExtra("Description", "Timed Sudoku settled through the generic Morpheus oracle kernel: puzzle sealed in the TEE, kernel-verified finalize result, penalized undos, pull-payment winnings.")]
    ...
    public partial class MiniAppSudoku : SmartContract
    {
        ...
        {
            if (d == 0) return 900_000;     // 15 min
            if (d == 1) return 1_500_000;   // 25 min
            return 2_400_000;               // 40 min
        }

        private static BigInteger MinSolveMsOf(BigInteger d)
        {
            if (d == 0) return 90_000;      // bot-deterrent floor: 90s
            if (d == 1) return 150_000;     // 150s
            return 240_000;                 // 240s
        }

        private static BigInteger TargetScoreOf(BigInteger d)
        {
            if (d == 0) return 1;           // any solved board clears easy
            if (d == 1) return 1;
            return 1;                       // sudoku scores a solved board as 1
        }
        #endregion
