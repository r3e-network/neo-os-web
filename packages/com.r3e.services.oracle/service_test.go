...
func ExampleService_CreateRequest() {
	...
	svc := New(NewMemoryAccountChecker(store), NewMemoryStoreAdapter(store, store), log)
...
func TestService_Publish_InvalidPayload(t *testing.T) {
	...
	store := memory.New()
	acct, _ := store.CreateAccount(context.Background(), account.Account{Owner: "owner"})
	svc2 := New(NewMemoryAccountChecker(store), NewMemoryStoreAdapter(store, store), nil)
