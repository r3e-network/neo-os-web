...
`app.platformDeFi` now covers all 55 non-control methods in the current local PlatformDeFi manifest plus native NEO/GAS prepaid deposits. It auto-threads the host app id, defaults account arguments to the connected wallet, retains transaction-sent/event-wait recovery hooks, and applies the same guest → `invoke:primary` guard to every write. Native deposits must carry the exact `${appId}:credit` memo; direct credits are keyed by `(appId,payer)`, tracked by per-app and global NEO/GAS liabilities, recoverable through pause-immune witness-gated withdrawals, and protected by post-transfer solvency checks after every business payout. This is deliberately not called an adoption: the generated ledger records zero shared bindings, the retained testnet artifact is `live-artifact-drift`, and the existing SelfLoan, FlashLoan, and TimeCapsule apps expose materially different standalone ABIs and recovery state machines. A named first-tenant migration still requires exact ABI mapping, money-path invariants, drain/rollback proof, deployed-artifact equality, and a funded lifecycle before any manifest changes.
