...
- **`registerApp(string appId, string engineId, UInt160 appAdmin, Map<string,object> descriptor)`** — **permissionless**, appAdmin witness, fee consumed from the caller's prepaid GAS credit (the PlatformAnchor M-11 model — the only registration path that ever attracted self-service tenants). Two tiers (RMAA graft):
  ...
  - **Full:** also mints the AppAccount (§4); fee additionally covers Neo's ~10 GAS minimum deployment fee. `mintAccount(appId)` upgrades a lite registration later.
...
- `spendToPayout(appId, asset, amount)` — app-admin witness; destination is **only** the registered payout address, whose changes go through a 24h timelock (a compromised app-admin key cannot instantly redirect); amounts above a descriptor threshold require a timelocked `proposeSpend/executeSpend` pair.
...
- **`escapeExecute(asset, amount)`** — app-admin-witnessed direct exit, usable only after the registry has been globally paused ≥ the escape timelock, defaulting to 30 days to mirror `AA_REGISTRATION_ESCAPE_TIMELOCK_SECONDS` (`framework/utils/aa-account.ts:18`, bounds 7–90 days). A bricked or captured registry can never permanently trap app treasuries — the credible-exit guarantee whose absence drove the last defection. This closes RMAA's fleet-freeze flaw (live-only authority, no hatch).
- All witness-gated exits (engine `withdraw`, registry `withdrawCredit`, `spendToPayout`, `escapeExecute`) are **pause-immune** per the anchor invariant.
...
| 4 | **Registry brick freezes every treasury fleet-wide, no escape hatch** (live-only authority) (J1+J2, RMAA) | AppAccount caches appId/registry/appAdmin locally (push-refreshed via `OnAdminRotated`); `escapeExecute` opens after ≥30-day global pause (mirroring `AA_REGISTRATION_ESCAPE_TIMELOCK_SECONDS`); all exits pause-immune; the registry holds no pooled user funds. |
...
| 7 | **Factory/registry-gated fleet shim `Update` = single-key rug over all treasuries; platform recovery-rotation = timelocked seizure path** (J1+J2, Estate) | Shim upgrades require platform timelock **and** per-app consent flag; platform admin has zero spend path over app accounts (test-pinned); recovery rotation is retained only as the timelocked, event-emitting `setAppAdmin` pair whose blast radius is bounded by the escape hatch and the timelocked payout address. Documented as an accepted, visible trade-off. |
...
1. **Materialization economics.** Full registration costs ~10+ GAS (Neo's minimum deployment fee). Does the platform subsidize account minting for existing fleet apps, pass it through, or leave lite-tier as the default until an app wants a treasury?
