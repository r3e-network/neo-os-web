...
**Status update (2026-03-07):** Focused web env tests/build/typecheck, host-app build, focused Go boundary tests, and the expanded contract warning/build gates are green in this workspace. The contracts sweep now passes cleanly with `./contracts/build.sh`, and the broad repo validation gates `npm test --silent`, `dotnet test contracts/__tests__/NeoContracts.Tests.csproj -c Release -v q`, and `go test ./...` are all green as of March 7, 2026. The remaining known environment-dependent limitation is still NeoExpress Governance staking: NeoExpress fresh-chain deployment no longer fails with `Insufficient GAS`, but spendable NEO on named NeoExpress wallets is still inconsistent enough that the Governance staking portion of `test/contract/platform_contracts_integration_test.go` skips explicitly when NeoExpress reports `NEO transfer failed`.
