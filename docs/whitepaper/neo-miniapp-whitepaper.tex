...
\textbf{Historical whitepaper note.}
This whitepaper captures an earlier platform architecture snapshot.
The current implementation no longer uses \texttt{ServiceLayerGateway} as the
preferred runtime path and no longer assumes universal \texttt{PaymentHub}
settlement for every flagship MiniApp.
For current implementation truth, prefer \texttt{docs/ARCHITECTURE.md},
\texttt{docs/DATAFLOWS.md}, \texttt{docs/service-api.md}, and the latest
flagship live validation reports.%
}}
...
\begin{tikzpicture}[
    ...
    \node[layer] (tee) at (0,3) {\textbf{TEE Service Layer} (NitroRun + AWS Nitro)};
...
The Trusted Execution Environment Service Layer represents the security foundation of the Neo N3 MiniApp Platform, providing hardware-protected execution for all sensitive operations. This layer leverages Intel Software Guard Extensions (Nitro) technology through the Nitro runtime framework, with AWS Nitro providing orchestration, attestation, and secure secret distribution across enclave instances.
...
Key material within the TEE Service Layer is bound to specific enclave measurements through Nitro sealing, ensuring that keys cannot be extracted even by privileged system software. AWS Nitro's manifest-based configuration defines the expected measurements for each service, enabling remote attestation that verifies enclave integrity before secrets are provisioned.
...
AWS Nitro technology provides the hardware foundation for the platform's security model, enabling code execution within isolated enclaves that are protected from observation or manipulation by any software running outside the enclave, including the operating system and hypervisor. This protection extends to physical attacks, as enclave memory is encrypted by the CPU using keys that are not accessible to any external entity.

The platform leverages Nitro through the Nitro runtime framework, which enables Go applications to execute within Nitro enclaves with minimal modification. AWS Nitro provides the enclave isolation and attestation primitives, while the platform defines the expected measurements, signing keys, and configuration parameters for each service so enclave integrity can be verified before sensitive material is provisioned.

Key management within TEE services follows a hierarchical derivation model. A master key, generated within the enclave during initial provisioning, is sealed to the enclave measurement using Nitro sealing. Service-specific keys are derived from this master key using domain separation, ensuring that compromise of one service's keys does not affect others. Transaction signing keys are further derived per-account, enabling fine-grained access control and key rotation without affecting other accounts.

Side-channel attack mitigations are implemented throughout TEE services, including constant-time cryptographic operations, memory access pattern obfuscation, and careful management of enclave entry and exit points. While Nitro provides strong isolation guarantees, the platform's security model does not rely solely on Nitro, instead treating TEE protection as one layer within the broader defense-in-depth architecture.
...
The service continuously polls price data from multiple independent sources, including centralized exchanges, decentralized exchanges, and professional data providers. Within the Nitro enclave, the service computes median values across sources, effectively filtering outliers and preventing any single source from manipulating reported prices. A configurable deviation threshold, defaulting to 0.1\%, determines when price updates warrant on-chain anchoring, balancing data freshness against transaction costs.
...
The platform's defense-in-depth security architecture ensures that sensitive operations are protected at every layer, from client-side validation through on-chain enforcement. The use of AWS Nitro enclaves provides hardware-level guarantees that cannot be circumvented by software attacks, while the multi-layer validation approach ensures resilience against novel attack vectors.
