...
\begin{tikzpicture}[
    ...
    \node[layer] (tee) at (0,3) {\textbf{TEE Service Layer} (MarbleRun + AWS Nitro)};
...
The platform's defense-in-depth security architecture ensures that sensitive operations are protected at every layer, from client-side validation through on-chain enforcement. The use of AWS Nitro technology through the MarbleRun framework provides hardware-level guarantees that cannot be circumvented by software attacks, while the multi-layer validation approach ensures resilience against novel attack vectors.
