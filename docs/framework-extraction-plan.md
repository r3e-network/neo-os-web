...
- **Wave 6 — DONE** (`edfb0ee80`): cross-app source imports dissolved (profitanchor-admin/trustanchor-admin → `@shared/composables/{profitanchor,trustanchor}`; neo-pay-shared-example was already on `@shared/composables/neo-pay`); `TransferService`, `registerActions` (createActionHandlers), `useAllEvents`, and the positional `NotificationService.guard` path `@deprecated` toward `app.funds` / `actions.register` / `app.events.listAll` / `app.notify.guard` (consumers remain, so implementations kept); campaign-orphaned `useTicker` composable deleted; `createObservable`/`Observable` in `react/context.ts` now re-export the framework canonical.
- **Final sweep — DONE** (`5618978f0`): the 8 game apps predating the Wave-1 notify surface (aim-master, burn-league, fogplay, gas-lucky-pool, last-survivor, on-chain-tarot, red-envelope, sheep-solitaire) moved toasts → `app.notify` and address reads → `app.chain`; fogplay dead event-bus channels deleted; sudoku board-store + breakup-contract pact-meta → `app.storage.local` (legacy keys byte-identical); dice-game/neo-message EVM lanes exempt-tagged.

**CAMPAIGN COMPLETE.** Final acceptance (plan §4): `ctx.services.notify` = **0** repo-wide (code + comments); all surviving `ctx.services.chain` hits are EVM lanes under a §3.6 `framework-exempt` tag; `eventBus` references are comment-only; the single raw `localStorage` site (automation-copilot auth-header harvesting) is exempt-tagged; **33** total exemption tags. Suites: apps/shared 2659/2659, framework 291/291, 18/18 miniapp suites, phaser guard 8/8. Manual testnet flow checks for the Wave-4 deposit-then-act cohort (per-app checklists in the Wave-4 workflow results) remain the one human-gated item before mainnet confidence.
