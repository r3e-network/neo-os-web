...
const mockConsents: Record<string, number> = {};
const mockWalletState = {
  ...
  disconnect: jest.fn(),
  switchChain: jest.fn(),
  hasBridgeConsent: jest.fn((origin: string) => Boolean(mockConsents[origin])),
  grantBridgeConsent: jest.fn((origin: string) => {
    mockConsents[origin] = Date.now();
  }),
  revokeBridgeConsent: jest.fn((origin: string) => {
    delete mockConsents[origin];
  }),
};
...
describe("Bridge Handler", () => {
  beforeEach(() => {
    ...
    for (const k of Object.keys(mockConsents)) delete mockConsents[k];
  });
  ...
  describe("cross-origin read consent", () => {
    const FOREIGN = "https://miniapp.example.com";

    it("denies GET_ACCOUNT for an un-consented cross-origin embed", async () => {
      const result = await handleBridgeMessage(msg("MULTICHAIN_GET_ACCOUNT", { chainId: "neo-n3" }), {
        origin: FOREIGN,
      });
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe("CONSENT_REQUIRED");
      expect(mockWalletState.hasBridgeConsent).toHaveBeenCalledWith(FOREIGN);
    });

    it("allows GET_ACCOUNT after the origin is granted consent", async () => {
      mockWalletState.grantBridgeConsent(FOREIGN);
      const result = await handleBridgeMessage(msg("MULTICHAIN_GET_ACCOUNT", { chainId: "neo-n3" }), {
        origin: FOREIGN,
      });
      expect(result.success).toBe(true);
    ...
    it("denies opaque 'null' origin even though it cannot store consent", async () => {
      const result = await handleBridgeMessage(msg("MULTICHAIN_GET_ACCOUNT", { chainId: "neo-n3" }), {
        origin: "null",
      });
      expect(result.success).toBe(false);
    ...
    it("denies GET_ACTIVE_CHAIN for an un-consented cross-origin embed", async () => {
      const result = await handleBridgeMessage(msg("MULTICHAIN_GET_ACTIVE_CHAIN"), { origin: FOREIGN });
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe("CONSENT_REQUIRED");
    });

    it("denies CONNECT for an un-consented cross-origin embed", async () => {
      const result = await handleBridgeMessage(msg("MULTICHAIN_CONNECT", { chainId: "neo-n3" }), {
        origin: FOREIGN,
      });
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe("CONSENT_REQUIRED");
    ...
    it("denies GET_BALANCE (default address) for an un-consented cross-origin embed", async () => {
      const result = await handleBridgeMessage(msg("MULTICHAIN_GET_BALANCE", { chainId: "neo-n3" }), {
        origin: FOREIGN,
      });
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe("CONSENT_REQUIRED");
    ...
    it("first-party (no origin) reads remain allowed without consent", async () => {
      const result = await handleBridgeMessage(msg("MULTICHAIN_GET_ACCOUNT", { chainId: "neo-n3" }));
      expect(result.success).toBe(true);
      expect(mockWalletState.hasBridgeConsent).not.toHaveBeenCalled();
    });

    it("READ_CONTRACT (non-identity) is not gated for cross-origin embeds", async () => {
      const result = await handleBridgeMessage(
        msg("MULTICHAIN_READ_CONTRACT", {
          chainId: "neo-n3",
          contractAddress: "0xabc",
