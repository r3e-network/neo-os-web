describe("morpheus runtime catalog", () => {
  it("exposes Morpheus workflow topology through the external integration registry", () => {
    ...
    expect(EXTERNAL_INTEGRATIONS.testnet.morpheusWorkflowIds).toContain(
      "automation.upkeep",
    );
    expect(EXTERNAL_INTEGRATIONS.testnet.morpheusEnvelopeVersion).toBe(
      "2026-04-tee-v1",
    );
    expect(EXTERNAL_INTEGRATIONS.testnet.morpheusRiskPlane).toBe(
      "independent_observer",
    );
    expect(
      EXTERNAL_INTEGRATIONS.testnet.morpheusAutomationTriggerKinds,
    ).toEqual(["interval", "threshold"]);
    expect(
      MORPHEUS_PUBLIC_RUNTIME_CATALOG.networks.mainnet.morpheus.datafeedCvmId,
    ).toBe("ac5b6886a2832df36e479294206611652400178f");
    ...
    expect(
      workflows.find((item) => item.id === "paymaster.authorize"),
    ).toBeTruthy();
    expect(
      workflows.find((item) => item.id === "automation.upkeep")?.execution,
    ).toEqual({
      orchestrationPlane: "control_plane",
      executionPlane: "tee_runtime",
      riskPlane: "independent_observer",
