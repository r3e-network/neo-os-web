...
import { getErrorCount, getTrackedErrors, type TrackedError } from "@/lib/monitoring/errors";
...
export function PerformanceReportPanel({
  ...
  const latestMetrics = Object.entries(groupedMetrics).map(([name, values]) => ({
    name,
  ...
  return (
    <>
      ...
      {/* Panel */}
      {isOpen && (
        <div className={`fixed ${positionClasses[position]} mt-12 w-80 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden z-50`}>
          {/* Header */}
                    ...
                    value !== undefined && (
                      <div key={key} className="flex justify-between items-center text-sm">
                        ...
                        <span className={`font-mono ${
                          ...
                          {value.toFixed(2)}{getVitalUnit(key)}
                        </span>
                      </div>
                    )
                  ))}
            ...
            {report && (
              <div className="text-xs text-gray-500 dark:text-gray-400">
                <div className="flex justify-between">
                  <span>URL:</span>
                  <span className="font-mono truncate max-w-[150px]" title={report.url}>
...
function ErrorList() {
  ...
  if (errors.length === 0) {
    return <p className="text-xs text-gray-500 dark:text-gray-400">No errors tracked</p>;
  }
  ...
  return (
    <div className="space-y-1">
      {errors.map((error) => (
        <div
          key={error.id}
          className="text-xs p-2 bg-gray-50 dark:bg-gray-900 rounded truncate"
          title={error.message}
        >
 ...
 */
function getVitalStatus(key: string, value: number): { status: string; color: string } {
  ...
  if (value <= threshold.poor) {
    return { status: "needs-improvement", color: "text-yellow-600 dark:text-yellow-400" };
  }
