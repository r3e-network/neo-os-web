...
export function CodeBlock({ code, language = "bash" }: CodeBlockProps) {
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard.writeText(code).catch((e: unknown) => {
      console.warn(
        "[CodeBlock] clipboard write failed:",
  ...
  return (
    <div className="group relative my-6 overflow-hidden rounded-xl border border-gray-200 bg-gray-50 shadow-sm">
        ...
        >
          {copied ? (
            <>
              ...
              <span className="text-neo">Copied</span>
            </>
          ) : (
            <>
              <Copy size={12} />
      ...
      <pre className="overflow-x-auto p-5 text-[13px] leading-[1.65]">
        <code className="font-mono text-gray-700">{code}</code>
      </pre>
    </div>
  );
}
