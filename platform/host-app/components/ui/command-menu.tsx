      ...
      <div className="fixed left-1/2 top-1/4 -translate-x-1/2 w-full max-w-lg cursor-default" onClick={(e) => e.stopPropagation()}>
        <Command className="rounded-xl border border-gray-200 bg-white shadow-2xl">
          <div className="flex items-center border-b border-gray-200 px-3">
            ...
            <Command.Input
              placeholder="Search..."
              className="h-12 w-full bg-transparent text-gray-900 placeholder:text-gray-400 focus-visible:outline-none"
            />
          </div>
          <Command.List className="max-h-80 overflow-y-auto p-2">
            ...
            {commands.map((cmd) => (
              <Command.Item
                ...
                className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-gray-700 cursor-pointer hover:bg-gray-100 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neo/50"
              >
                <cmd.icon size={16} />
                {cmd.label}
              </Command.Item>
