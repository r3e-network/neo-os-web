      ...
      {isOpen && (
        ...
        >
          ...
          <div className="flex border-b border-gray-200/50">
            {(["overview", "errors", "performance", "analytics"] as const).map(
              (tab) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`flex-1 py-3 px-3 text-xs font-bold capitalize transition-all focus-visible:outline-none ${
                    activeTab === tab
          ...
          <div className="p-4 max-h-[70vh] overflow-y-auto">
            ...
            {activeTab === "analytics" && (
              <AnalyticsTab session={session} events={bufferedEvents} />
            )}
          </div>
        </div>
      )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        />
        <StatCard label="Metrics" value={metricsCount} color="text-blue-600" />
        <StatCard label="Events" value={eventsCount} color="text-purple-600" />
        <StatCard
          label="Page Views"
      ...
      {errors.map((error) => (
        ...
        >
          <div className="flex items-start justify-between mb-1">
            ...
            <span className="text-xs text-gray-500">x{error.count}</span>
          </div>
          <p className="text-sm font-medium text-gray-900 mb-1">{error.name}</p>
  ...
  return (
    <div className="space-y-3">
      {latest.map((metric) => (
        <div
          key={metric.name}
          className="bg-white/50 border border-gray-200/50 rounded-2xl p-4"
        >
      ...
      <div className="bg-gray-50 rounded-lg p-3">
        ...
        <div className="flex flex-wrap gap-1">
          {Object.entries(eventTypes).map(([type, count]) => (
            <span key={type} className="text-xs bg-gray-200 px-2 py-1 rounded">
              {type}: {count}
            </span>
          ))}
        </div>
