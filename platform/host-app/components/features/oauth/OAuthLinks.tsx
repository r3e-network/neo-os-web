...
export function OAuthLinks() {
  const { accounts, loading, error, linkAccount, unlinkAccount, clearError } =
    useOAuthStore();

  const isLinked = (provider: OAuthProvider) =>
    accounts.some((a) => a.provider === provider);

  const getAccount = (provider: OAuthProvider) =>
      ...
      {error && (
        ...
        >
          ...
          <button
            type="button"
            onClick={clearError}
            className="mt-1 text-xs text-red-500 underline cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/50 rounded-lg"
          >
            Dismiss
          </button>
        </div>
