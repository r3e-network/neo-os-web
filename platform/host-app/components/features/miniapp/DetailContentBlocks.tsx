        ...
        if (block.type === "notice") {
          return (
            <section
              ...
              className={`rounded-xl border px-4 py-3 ${NoticeToneClass(block.tone)}`}
            >
              {block.title && (
              ...
              <dl className="space-y-1.5">
                {block.items && block.items.length > 0
                  ? block.items.map((item, itemIndex) => (
                      <div
                        key={itemIndex}
                        className="flex items-start justify-between gap-4 border-b border-gray-200/70 pb-1.5 last:border-b-0 last:pb-0"
                      >
                        <dt className="text-sm text-gray-500">{item.key}</dt>
                        <dd className="text-sm text-gray-900 text-right break-words">
                          {item.value}
                        </dd>
                      </div>
                    ))
