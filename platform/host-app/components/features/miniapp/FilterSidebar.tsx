        ...
        {sections.map((section) => (
          <div key={section.id} className="mb-4">
            <button
              ...
              className="flex items-center justify-between w-full text-left py-2 text-sm font-medium text-gray-900 dark:text-white hover:text-neo dark:hover:text-neo transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neo/50"
            >
              {section.label}
              {expanded[section.id] ? (
                ...
                {section.options.map((option) => {
                  ...
                  return (
                    <label
                      key={option.value}
                      className={cn(
                        "flex items-center justify-between gap-2 px-3 py-2 rounded-md cursor-pointer text-sm transition-colors group",
                        isSelected
