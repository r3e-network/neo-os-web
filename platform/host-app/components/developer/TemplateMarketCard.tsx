...
export function TemplateMarketCard({
  ...
  return (
    <div className="rounded-3xl border border-gray-200/50 bg-white/60 p-5 backdrop-blur-xl transition-all hover:border-neo/40 hover:shadow-[0_10px_30px_rgba(0,229,153,0.1)]">
      ...
      <p className="text-sm font-semibold text-gray-900">{name || templateId}</p>
      ...
      {tags.length > 0 ? (
        <div className="mt-2 flex flex-wrap gap-1">
          {tags.slice(0, 4).map((tag) => (
            <span key={`${templateId}-${tag}`} className="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] text-gray-600">
              {tag}
            </span>
          ))}
        </div>
