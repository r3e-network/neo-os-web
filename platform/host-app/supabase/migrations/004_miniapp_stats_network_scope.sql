...
BEGIN
  ...
  RETURN QUERY
  WITH sim AS (
    SELECT st.app_id::TEXT AS app_id, COUNT(*)::BIGINT AS tx_count, COUNT(DISTINCT st.account_address)::BIGINT AS user_count, COALESCE(SUM(st.amount), 0)::NUMERIC AS volume
    FROM public.simulation_transactions st
    WHERE st.app_id IS NOT NULL AND st.network = p_network AND (p_app_id IS NULL OR st.app_id = p_app_id)
    GROUP BY st.app_id
  ),
  svc AS (
    SELECT sr.app_id::TEXT AS app_id, COUNT(*)::BIGINT AS tx_count, COUNT(DISTINCT sr.requester)::BIGINT AS user_count
    FROM public.service_requests sr
    WHERE sr.app_id IS NOT NULL AND sr.network = p_network AND (p_app_id IS NULL OR sr.app_id = p_app_id)
    GROUP BY sr.app_id
  ),
  evt AS (
    SELECT ce.app_id::TEXT AS app_id, COUNT(*)::BIGINT AS tx_count
    FROM public.contract_events ce
    WHERE ce.app_id IS NOT NULL AND ce.network = p_network AND (p_app_id IS NULL OR ce.app_id = p_app_id)
    GROUP BY ce.app_id
  ),
  all_apps AS (
  ...
  SELECT
    a.app_id::TEXT,
    (COALESCE(sim.user_count, 0) + COALESCE(svc.user_count, 0))::BIGINT,
    (COALESCE(sim.tx_count, 0) + COALESCE(svc.tx_count, 0) + COALESCE(evt.tx_count, 0))::BIGINT,
    (COALESCE(sim.volume, 0) / 100000000)::NUMERIC
  FROM all_apps a
