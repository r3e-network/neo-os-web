        ...
        <div className="w-full max-w-sm space-y-6 rounded-2xl bg-white dark:bg-gray-900 p-8 shadow-lg dark:shadow-none dark:border dark:border-gray-800">
          ...
          <div className="space-y-3">
            {socialProviders.map((p) => (
              <button
                ...
                disabled={loading}
                className={`flex w-full items-center justify-center gap-2 rounded-lg px-4 py-3 font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neo/50 ${p.bg} ${p.text}`}
              >
          ...
          {error && (
            <div role="alert" className="rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-3 text-sm text-red-600 dark:text-red-400">
              {error}
              <button type="button" onClick={clearError} className="ml-2 underline transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/50 rounded-lg">dismiss</button>
            </div>
          )}
        </div>
      </main>
