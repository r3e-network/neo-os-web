...
import { APP_ID_REGEX, asTrimmedString, getQueryString, parseReleaseChannel } from "@/pages/api/miniapps/admin/version-utils";
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!hasServiceRoleSupabase()) {
    return apiError.configError(res, "SUPABASE_SERVICE_ROLE_KEY is required for admin miniapp reads");
  }
  ...
  const releaseChannel = parseReleaseChannel(getQueryString(req, "release_channel"));
  ...
  try {
    const includePayload = asTrimmedString(req.query.include_payload).toLowerCase() === "true";
    ...
    logger.error("miniapp admin versions list failed:", error instanceof Error ? error.message : "unknown error");
