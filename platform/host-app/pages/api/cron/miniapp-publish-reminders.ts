...
async function callPublishReminderEndpoint(baseUrl: string, cronSecret: string): Promise<{
  ok: boolean;
  status: number;
  payload: unknown;
}> {
  const response = await fetch(`${baseUrl}/api/miniapps/admin/publish-reminders`, {
    method: "POST",
    headers: {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const baseUrl = String(process.env.HOST_APP_BASE_URL || process.env.NEXT_PUBLIC_HOST_APP_BASE_URL || "").trim();
  if (!baseUrl) {
    return apiError.configError(res, "HOST_APP_BASE_URL is required for publish reminder cron");
  }
  ...
  try {
    ...
    return apiError.internal(
      res,
      error instanceof Error ? `Publish reminder cron error: ${error.message}` : "Publish reminder cron failed",
    );
  }
}
