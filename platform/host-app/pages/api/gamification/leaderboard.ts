...
export default async function handler(
  ...
  try {
    ...
    if (!hasServiceRoleSupabase()) {
      res.status(200).json({ entries: [], total: 0, hasMore: false });
      return;
    }
    ...
    if (!error && Array.isArray(data)) {
      const entries: LeaderboardEntry[] = (data as LeaderboardRow[]).map(
        (row) => ({
    ...
    if (error) {
      logger.warn(
        "get_gamification_leaderboard RPC failed, using query fallback:",
        error.message,
      );
    }
    ...
    if (usageError) {
      logger.error(
        "miniapp_usage leaderboard fallback failed:",
        usageError.message,
      );
      res.status(200).json({ entries: [], total: 0, hasMore: false });
      return;
    ...
    const aggregate = new Map<
      string,
      { totalTx: number; totalVotes: number; appIds: Set<string> }
    >();
    ...
    const unresolvedUserIds = userIdList.filter(
      (id) => !walletByUserId.has(id),
    );
    ...
    res.status(500).json({ entries: [], total: 0, hasMore: false });
    return;
  }
}
