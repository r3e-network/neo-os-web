...
export async function GET(req: Request) {
  ...
  try {
    const upstream = new URL("/api/miniapps/admin/template-catalog", hostAppBaseURL);
    ...
    if (!response.ok) {
      const message = await parseHostErrorPayload(response, "Failed to load template catalog");
