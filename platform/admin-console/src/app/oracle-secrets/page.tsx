    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">Manage API keys and tokens injected by the TEE Oracle</p>
        </div>
        ...
        <CardContent className="p-0">
          <table className="w-full text-left text-sm text-gray-300" aria-label="Oracle secrets">
            ...
            <tbody className="divide-y divide-white/5">
              ...
              {secrets.length === 0 && (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-gray-500">No secrets configured.</td>
                </tr>
              )}
            </tbody>
          </table>
...
{`// Payload for NeoOracle /query endpoint
{
  ...
  "secret_as_key": "X-MBX-APIKEY"
          ...
          <p className="text-sm text-gray-400 mt-4">
            The NeoOracle service executes this request inside the TEE and securely injects the actual API key mapped to <code>secret_name</code> into the header specified by <code>secret_as_key</code> before firing the HTTP request to Binance. This cleanly isolates PriceFeeds (data aggregation) from generic Oracle (request-response authenticated fetch).
          </p>
        </CardContent>
      </Card>
    </div>
