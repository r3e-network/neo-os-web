...
export function LayoutTab({
  ...
  return (
    <div className="space-y-6">
      <div>
        <div className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Hero</div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <div className="text-sm font-medium text-gray-700 dark:text-gray-300">Tabs</div>
        ...
        {dtTabs.map((t, i) => (
          <div key={i} className="rounded-lg border border-gray-200 dark:border-gray-700 p-3 mb-3 space-y-2">
            <div className="flex gap-2 items-center">
              <Input id={`tab-id-${i}`} placeholder="ID" value={t.id} onChange={e => { const next = [...dtTabs]; next[i] = { ...next[i], id: e.target.value }; updateDT({ tabs: next }); }} aria-label="Tab ID" />
              <Input id={`tab-label-${i}`} placeholder="Label" value={t.label} onChange={e => { const next = [...dtTabs]; next[i] = { ...next[i], label: e.target.value }; updateDT({ tabs: next }); }} aria-label="Tab label" />
              <select id={`tab-type-${i}`} className="rounded-md border border-gray-300 dark:border-gray-600 p-1.5 text-xs cursor-pointer dark:bg-gray-800 dark:text-gray-100 w-32 shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/50" value={t.type} onChange={e => { const next = [...dtTabs]; next[i] = { ...next[i], type: e.target.value }; updateDT({ tabs: next }); }} aria-label="Tab type">
      ...
      <div>
        <div className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Operation Panel</div>
      ...
      <div>
        <div className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Frontend Spec (Optional)</div>
        <div className="grid grid-cols-4 gap-4 mb-2">
          <div>
            <label htmlFor="frontend-spec-format" className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Format</label>
            <select
          ...
          <div className="col-span-3">
            <div className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Spec Content</div>
            <textarea
