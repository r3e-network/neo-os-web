...
import { useSimulationStatus, useStartSimulation, useStopSimulation } from "@/lib/hooks/useSimulation";
...
export default function SimulationsPage() {
  ...
  const handleStart = () => {
    const apps = config.mini_apps.split(',').map(s => s.trim()).filter(Boolean);
        ...
        <CardContent>
          ...
          <div className="flex flex-col space-y-4 mb-8">
            ...
            <div className="flex space-x-4">
              <button
                onClick={handleStart}
                disabled={isLoading || status?.running || startSimulation.isPending}
              ...
              <button
                onClick={handleStop}
                disabled={isLoading || !status?.running || stopSimulation.isPending}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded">
                ...
                <div className="text-xl font-bold">{status?.active_miniapps?.length || 0}</div>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded">
                <div className="text-sm text-gray-500">Workers per App</div>
                <div className="text-xl font-bold">{status?.workers_per_app || 0}</div>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded">
                <div className="text-sm text-gray-500">Uptime (s)</div>
                <div className="text-xl font-bold">{status?.uptime_seconds || 0}</div>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded">
                <div className="text-sm text-gray-500">Transactions Simulated</div>
