// Package main provides the generic Nitro entry point for all Neo services.
// The service type is determined by the NITRO_TYPE environment variable.
// Each service is a separate Nitro in NitroRun, running in its own TEE enclave.
package main
...
func main() {
	...
	// Get service type from environment (injected by NitroRun manifest)
	serviceType := strings.TrimSpace(os.Getenv("NITRO_TYPE"))
	if serviceType == "" {
		serviceType = strings.TrimSpace(os.Getenv("SERVICE_TYPE")) // Fallback for local testing
	}
	if serviceType == "" {
		log.Fatalf("NITRO_TYPE environment variable required. Available services: %v", availableServices)
	}
	if !isAvailableService(serviceType) {
		log.Fatalf("Unknown NITRO_TYPE %q. Available services: %v", serviceType, availableServices)
	}
	...
	m, err := nitro.New(nitro.Config{
		NitroType: serviceType,
	})
	if err != nil {
		log.Fatalf("Failed to create nitro: %v", err)
	}

	// Initialize Nitro with Coordinator
	if initErr := m.Initialize(ctx); initErr != nil {
		log.Fatalf("Failed to initialize nitro: %v", initErr)
	}
	...
	if runtime.StrictIdentityMode() && m.TLSConfig() == nil {
		log.Fatalf("CRITICAL: mTLS credentials are required in strict identity mode (missing NITRO_CERT/NITRO_KEY/NITRO_ROOT_CA)")
	}
