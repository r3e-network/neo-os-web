import React from 'react';
import { FiZap, FiCheckCircle, FiClock, FiAlertCircle } from 'react-icons/fi';
import { Card } from '@/components/common';

interface AutomationStatsProps {
  totalTriggers: number;
  active: number;
  executionsToday: number;
  errors: number;
}

export function AutomationStats({ totalTriggers, active, executionsToday, errors }: AutomationStatsProps) {
  const stats = [
    { label: 'Total Triggers', value: totalTriggers, icon: FiZap, color: 'text-yellow-600' },
    { label: 'Active', value: active, icon: FiCheckCircle, color: 'text-green-600' },
    { label: 'Runs Today', value: executionsToday, icon: FiClock, color: 'text-blue-600' },
    { label: 'Errors', value: errors, icon: FiAlertCircle, color: 'text-red-600' },
  ];

  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {stats.map((stat) => (
        <Card key={stat.label}>
          <div className="flex items-center gap-3">
            <div className={`p-2 bg-surface-100 rounded-lg ${stat.color}`}>
              <stat.icon className="w-5 h-5" />
            </div>
            <div>
              <p className="text-sm text-surface-500">{stat.label}</p>
              <p className="text-xl font-bold text-surface-900">{stat.value}</p>
            </div>
          </div>
        </Card>
      ))}
    </div>
  );
}
