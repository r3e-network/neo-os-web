...
export const MICROGAME_ARCHETYPES = [
  ...
  },
  {
    id: "timber-chop",
    name: "Timber Chop",
    familiarPattern: "Side-switch tree chopping arcade challenges",
    coreVerb: "tap",
    sessionSeconds: [20, 45],
    verification: "Replay obstacle seed, chop direction inputs, and trunk collision checks.",
    gamefiFit: "high",
    primarySkillSignal: "Fast left/right decisions while avoiding branch collisions.",
    playSurface: "A central trunk with the character, branches, timer, and combo streak as the hero.",
    controlModel: "Tap left or right side of the playfield; no exposed parameter form.",
    assetDirection: "Character chop poses, trunk segments, branch warnings, wood-chip bursts, and combo flame.",
    antiAbuse: [
      "Generate branch sequence from TEE seed.",
      "Record every side tap timestamp and direction.",
      "Reject impossible chop cadence and branch-collision mismatches.",
    ],
    modeTemplates: DEFAULT_MODES,
    implementationNotes: [
      "Good candidate for short paid attempts because every input is deterministic and replayable.",
      "Secondary finance details should remain in framework drawers; the trunk stays primary.",
    ],
  },
  {
    id: "fruit-slice-sprint",
    name: "Fruit Slice Sprint",
    familiarPattern: "Fruit-slicing swipe arcade challenges",
    coreVerb: "swipe",
    sessionSeconds: [20, 45],
    verification: "Replay object spawn seed, swipe paths, slice intersections, and bomb collisions.",
    gamefiFit: "high",
    primarySkillSignal: "Swipe accuracy, path timing, combo routing, and hazard avoidance.",
    playSurface: "A bright cutting board or sky arc where flying objects dominate the whole scene.",
    controlModel: "Swipe through objects; paid mode hides raw spawn controls behind difficulty routes.",
    assetDirection: "Glossy sliceable objects, cut halves, juice trails, hazard warnings, and combo ribbons.",
    antiAbuse: [
      "Sample swipe paths at fixed intervals.",
      "Verify slice intersections against deterministic object physics.",
      "Reject hidden-tab attempts and impossible multi-object slice paths.",
    ],
    modeTemplates: DEFAULT_MODES,
    implementationNotes: [
      "Strong mobile-native control model and visually rewarding asset surface.",
      "Needs clear hazard contrast so background art never competes with objects.",
    ],
  },
  {
    id: "brick-blitz",
    name: "Brick Blitz",
    familiarPattern: "Breakout-style brick breaker",
    coreVerb: "aim",
    sessionSeconds: [30, 90],
    verification: "Replay launch input, paddle samples, brick layout seed, and ball collisions.",
    gamefiFit: "high",
    primarySkillSignal: "Angle planning, paddle control, and efficient brick clears.",
    playSurface: "A clean brick board with the paddle and ball occupying the main canvas.",
    controlModel: "Drag paddle or aim-and-release; advanced stats belong in a drawer.",
    assetDirection: "Polished bricks, paddle skins, ball trails, power-up capsules, and impact particles.",
    antiAbuse: [
      "Derive brick layout and power-ups from TEE seed.",
      "Clamp paddle velocity and sample positions at fixed ticks.",
      "Score only verifier-replayed brick hits and power-up pickups.",
    ],
    modeTemplates: DEFAULT_MODES,
    implementationNotes: [
      "Works well for achievement ladders around perfect clears and no-miss rounds.",
      "Round length can be capped by bricks cleared rather than raw elapsed time.",
    ],
  },
  {
    id: "lock-pin-timing",
    name: "Lock Pin Timing",
    familiarPattern: "Rotating lock / pin timing challenges",
    coreVerb: "tap",
    sessionSeconds: [15, 45],
    verification: "Replay lock rotation seed and pin launch timestamps against occupied angles.",
    gamefiFit: "high",
    primarySkillSignal: "Timing precision across shrinking safe gaps.",
    playSurface: "A large rotating lock medallion with pins, safe gaps, and streak feedback.",
    controlModel: "Tap to launch one pin at a time; difficulty changes rotation and pin count.",
    assetDirection: "Neo medallion lock, metallic pins, safe-gap glow, collision sparks, and success seal.",
    antiAbuse: [
      "Compute rotation angle deterministically from seed and elapsed time.",
      "Reject duplicate or out-of-window launch timestamps.",
      "Validate angular collision radius consistently in client and verifier.",
    ],
    modeTemplates: DEFAULT_MODES,
    implementationNotes: [
      "Small implementation surface but demands high animation polish.",
      "Can share timing verifier primitives with knife-timing.",
    ],
  },
