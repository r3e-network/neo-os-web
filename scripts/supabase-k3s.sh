...
generate_secrets() {
    ...
    local database_url="postgresql://postgres:${postgres_password}@postgres.supabase.svc.cluster.local:5432/postgres"
...
create_postgres_roles() {
    ...
    kubectl exec -n supabase "$postgres_pod" -- psql -U postgres -d postgres <<'EOSQL' || {
...
deploy_supabase() {
    ...
    kubectl apply -k "$SUPABASE_DIR/overlays/local"
...
update_service_layer_config() {
    ...
    # Update ConfigMap
    kubectl patch configmap service-layer-config -n service-layer --type merge -p "{\"data\":{\"SUPABASE_URL\":\"http://postgrest.supabase.svc.cluster.local:3000\"}}" || {
