...
warn_env_file_placeholders() {
  ...
  if grep -Eq '^SUPABASE_SERVICE_KEY=***REMOVED***' "$path"; then
    echo "WARNING: SUPABASE_SERVICE_KEY in $path still uses the sample value from .env.example (***REMOVED***)." >&2
  fi
  if grep -Eq '^JWT_SECRET=***REMOVED***' "$path"; then
...
build_signed_images() {
  local packages=(gateway neofeeds neoflow neoaccounts neocompute neooracle globalsigner)
  local pkg
