...
for (const file of files) {
  ...
  const items = raw
    ...
    .map((item) => {
      const url = asTrimmedString(item.url) || asString(item.url);
      if (!url) return null;
      const themeRaw = (asTrimmedString(item.theme) || asString(item.theme)).toLowerCase();
      const densityRaw = (asTrimmedString(item.density) || asString(item.density)).toLowerCase();
      const locale = asTrimmedString(item.locale) || asOptionalString(item.locale);
      const res: any = { url };
