...
func main() {
	...
	payload := strings.TrimSpace(os.Getenv("MINIAPP_RNG_PAYLOAD"))
	...
	fmt.Println("=== MiniApp RNG Request (via ServiceLayerGateway) ===")
	fmt.Printf("RPC: %s\n", rpcURL)
	...
	testResult, err := act.Call(gatewayHash, "requestService", appID, "rng", payloadBytes, callbackContract, callbackMethod)
	if err != nil {
	...
	txHash, vub, err := act.SendCall(gatewayHash, "requestService", appID, "rng", payloadBytes, callbackContract, callbackMethod)
	if err != nil {
