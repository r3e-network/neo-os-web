...
type Service struct {
	*commonservice.BaseService
	...
	// Master seed (injected via NitroRun)
	masterSeed []byte
...
func New(cfg Config) (*Service, error) {
	...
	requireAttestation := cfg.Nitro != nil && cfg.Nitro.IsEnclave()
	...
	// Attach ServeMux routes to the nitro router.
...
func (s *Service) hydrate(ctx context.Context) error {
	...
	// Load master seed from Nitro secrets
	seedBytes, ok := s.Nitro().Secret("GLOBALSIGNER_MASTER_SEED")
	if !ok || len(seedBytes) == 0 {
		strict := runtime.StrictIdentityMode() || s.Nitro().IsEnclave()
		if strict {
