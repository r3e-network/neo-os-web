...
func TestRequestAccountsWithMock(t *testing.T) {
	...
	svc, err := New(Config{Nitro: m, NeoAccountsRepo: mockRepo})
	if err != nil {
		t.Fatalf("New() error = %v", err)
	}
...
func TestUpdateBalanceWithMockMultiToken(t *testing.T) {
	...
	svc, _ := New(Config{Nitro: m, NeoAccountsRepo: mockRepo})
	ctx := context.Background()
...
func TestHandleListAccountsMissingServiceID(t *testing.T) {
	m, _ := nitro.New(nitro.Config{NitroType: "neoaccounts"})
	m.SetTestSecret("POOL_MASTER_KEY", []byte("test-master-key-32-bytes-long!!!"))
	svc, _ := New(Config{Nitro: m, NeoAccountsRepo: newMockNeoAccountsRepo()})
