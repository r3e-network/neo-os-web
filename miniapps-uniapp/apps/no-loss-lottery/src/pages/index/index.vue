<template>
  <view class="app-container">
    ...
    <view class="card">
      ...
      <view class="row"
        ><text>Total deposits</text><text class="v">{{ formatNum(pool.totalDeposits, 0) }} GAS</text></view
      >
      <view class="row"
        ><text>Prize pool</text><text class="v">{{ formatNum(pool.prizePool, 2) }} GAS</text></view
      >
      <view class="row"
    ...
    <view class="card">
      ...
      <view class="row"
        ><text>Deposit</text><text class="v">{{ formatNum(user.deposit, 2) }} GAS</text></view
      >
      <view class="row"
        ><text>Tickets</text><text class="v">{{ user.tickets }}</text></view
      >
      <view class="row"
        ><text>Yield sacrificed</text><text class="v">{{ formatNum(user.yieldSacrificed, 3) }} GAS</text></view
      >
    </view>
...
const joinLottery = async (): Promise<void> => {
  ...
  try {
    ...
    status.value = { msg: `Deposited ${formatNum(amount, 2)} GAS, earned ${tickets} tickets`, type: "success" };
  } catch (e: any) {
