...
// falls back to environment variables and nitro secrets in order.
func resolveContractHash(cfgValue string, m *nitro.Nitro, envKeys ...string) string {
...
// environment variables, and nitro secrets.
...
func New(cfg Config) (*Service, error) { //nolint:gocritic // cfg is read once at startup.
	if cfg.Nitro == nil {
		return nil, fmt.Errorf("neorequests: nitro is required")
	}
