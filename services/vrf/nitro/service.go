...
func New(cfg Config) (*Service, error) {
	if cfg.Nitro == nil {
		return nil, fmt.Errorf("neovrf: nitro is required")
	}
	...
	s.attestationHash = computeAttestationHash(cfg.Nitro)

	if key, ok := cfg.Nitro.Secret("NEOVRF_SIGNING_KEY"); ok && len(key) >= 32 {
		s.signingKey = key
	} else if strict {
