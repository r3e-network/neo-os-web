...
The NeoOracle service provides a secure HTTP proxy for fetching external data from within the AWS Nitro TEE. It enforces an outbound URL allowlist and can inject user-owned secrets into outbound requests (for authenticated APIs).
...
- All outbound requests originate from within the AWS Nitro TEE (attested identity via mTLS)
