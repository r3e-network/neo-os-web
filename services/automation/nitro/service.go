...
func New(cfg Config) (*Service, error) { //nolint:gocritic // cfg is read once at startup.
	if cfg.Nitro == nil {
		return nil, fmt.Errorf("nitro is required")
	}
...
func (s *Service) schedulerHydrationUserID() string {
	...
	if nitroInstance := s.Nitro(); nitroInstance != nil {
		if value, ok := nitroInstance.Secret("NEOFLOW_SCHEDULER_USER_ID"); ok && len(value) > 0 {
			return string(value)
		}
	}
