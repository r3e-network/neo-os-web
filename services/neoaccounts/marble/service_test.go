...
func TestRequestAccountsWithMock(t *testing.T) {
	...
	accounts, lockID, err := svc.RequestAccounts(ctx, "neovault", 3, "mixing operation")
	if err != nil {
	...
	for _, acc := range accounts {
		if acc.LockedBy != "neovault" {
			t.Errorf("account %s should be locked by neovault", acc.ID)
		}
		if acc.Balances == nil {
...
func TestUpdateBalanceWithMockMultiToken(t *testing.T) {
	...
	// Update GAS balance
	oldBalance, newBalance, txCount, err := svc.UpdateBalance(ctx, "neovault", "acc-1", TokenTypeGAS, 500000, nil)
	if err != nil {
	...
	oldNeo, newNeo, txCount2, err := svc.UpdateBalance(ctx, "neovault", "acc-1", TokenTypeNEO, 10, nil)
	if err != nil {
...
func TestListAccountsByServiceWithMockMultiToken(t *testing.T) {
	...
	// List all accounts for neovault
	accounts, err := svc.ListAccountsByService(ctx, "neovault", "", nil)
	if err != nil {
	...
	minBal := int64(800000)
	filtered, err := svc.ListAccountsByService(ctx, "neovault", TokenTypeGAS, &minBal)
	if err != nil {
		t.Fatalf("ListAccountsByService() error = %v", err)
	}
...
func TestSignTransactionWithMock(t *testing.T) {
	...
	resp, err := svc.SignTransaction(ctx, "neovault", "acc-1", txHash)
	if err != nil {
...
func TestSignTransactionAccountNotFound(t *testing.T) {
	...
	txHash := crypto.Hash256([]byte("test"))
	_, err := svc.SignTransaction(ctx, "neovault", "nonexistent", txHash)
	if err == nil {
...
func TestBatchSignWithMock(t *testing.T) {
	...
	mockRepo := newMockNeoAccountsRepo()
	mockRepo.accounts["acc-1"] = &neoaccountssupabase.Account{ID: "acc-1", LockedBy: "neovault"}
	mockRepo.accounts["acc-2"] = &neoaccountssupabase.Account{ID: "acc-2", LockedBy: "neovault"}
	...
	resp := svc.BatchSign(ctx, "neovault", requests)
...
func TestReleaseAccountsWithMock(t *testing.T) {
	...
	released, err := svc.ReleaseAccounts(ctx, "neovault", []string{"acc-1", "acc-3"})
	if err != nil {
...
func TestReleaseAllByServiceWithMock(t *testing.T) {
	...
	released, err := svc.ReleaseAllByService(ctx, "neovault")
	if err != nil {
...
func TestUpdateBalanceInsufficientFunds(t *testing.T) {
	...
	_, _, _, err := svc.UpdateBalance(ctx, "neovault", "acc-1", TokenTypeGAS, -200, nil)
	if err == nil {
...
func TestUpdateBalanceAbsoluteValue(t *testing.T) {
	...
	// Set absolute value
	absValue := int64(999)
	oldBal, newBal, _, err := svc.UpdateBalance(ctx, "neovault", "acc-1", TokenTypeGAS, 0, &absValue)
	if err != nil {
		t.Fatalf("UpdateBalance() error = %v", err)
	}
...
func TestUpdateBalanceDefaultToken(t *testing.T) {
	...
	_, newBal, _, err := svc.UpdateBalance(ctx, "neovault", "acc-1", "", 100, nil)
	if err != nil {
		t.Fatalf("UpdateBalance() error = %v", err)
	}
	if newBal != 100 {
...
func TestHandleListAccountsEndpoint(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/accounts?service_id=neovault", nil)
	rr := httptest.NewRecorder()
...
func TestHandleListAccountsWithTokenFilter(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/accounts?service_id=neovault&token=GAS&min_balance=500000", nil)
	rr := httptest.NewRecorder()
...
func TestHandleRequestAccountsEndpoint(t *testing.T) {
	...
	body := `{"service_id": "neovault", "count": 1, "purpose": "test"}`
...
func TestHandleReleaseAccountsEndpoint(t *testing.T) {
	...
	body := `{"service_id": "neovault", "account_ids": ["acc-1"]}`
...
func TestHandleSignTransactionEndpoint(t *testing.T) {
	...
	body := fmt.Sprintf(`{"service_id": "neovault", "account_id": "acc-1", "tx_hash": %s}`, mustMarshalJSON(txHash))
...
func TestHandleBatchSignEndpoint(t *testing.T) {
	...
	txHash := crypto.Hash256([]byte("test tx"))
	body := fmt.Sprintf(`{"service_id": "neovault", "requests": [{"account_id": "acc-1", "tx_hash": %s}]}`, mustMarshalJSON(txHash))
...
func TestHandleUpdateBalanceEndpoint(t *testing.T) {
	...
	body := `{"service_id": "neovault", "account_id": "acc-1", "token": "GAS", "delta": 1000000}`
...
func TestHandleUpdateBalanceDefaultToken(t *testing.T) {
	...
	body := `{"service_id": "neovault", "account_id": "acc-1", "delta": 500}`
	req := httptest.NewRequest("POST", "/balance", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()
...
func TestRequestAccountsInvalidCount(t *testing.T) {
	...
	// Count 0 should fail
	_, _, err := svc.RequestAccounts(ctx, "neovault", 0, "test")
	if err == nil {
		t.Error("RequestAccounts should fail for count=0")
	}

	// Count > 100 should fail
	_, _, err = svc.RequestAccounts(ctx, "neovault", 101, "test")
	if err == nil {
...
func TestRequestAccountsNoAvailable(t *testing.T) {
	...
	accounts, _, err := svc.RequestAccounts(ctx, "neovault", 1, "test")
	if err != nil {
...
func TestHandleRequestAccountsDefaultCount(t *testing.T) {
	...
	body := `{"service_id": "neovault", "count": 0}`
	req := httptest.NewRequest("POST", "/request", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()
