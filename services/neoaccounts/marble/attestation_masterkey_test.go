...
func TestResolveServiceID_Behavior(t *testing.T) {
	...
	req = httptest.NewRequest(http.MethodGet, "/", nil)
	rr = httptest.NewRecorder()
	svc, ok := resolveServiceID(rr, req, "neovault")
	if !ok || svc != "neovault" {
		t.Fatalf("resolveServiceID() = (%q,%v), want (neovault,true)", svc, ok)
	}

	req = httptest.NewRequest(http.MethodGet, "/", nil)
	req.Header.Set("X-Service-ID", "neooracle")
	rr = httptest.NewRecorder()
	if _, ok := resolveServiceID(rr, req, "neovault"); ok {
...
func TestResolveServiceID_ProductionRejectsMismatch(t *testing.T) {
	...
	gen := middleware.NewServiceTokenGenerator(privateKey, "neovault", time.Hour)
	...
	auth := middleware.NewServiceAuthMiddleware(middleware.ServiceAuthConfig{
		...
		AllowedServices: []string{"neovault"},
	})
	...
	req.TLS = &tls.ConnectionState{
		VerifiedChains: [][]*x509.Certificate{{&x509.Certificate{DNSNames: []string{"neovault"}}}},
	}
