...
The NeoVault service provides privacy mixing for Neo N3 tokens (GAS, NEO). It uses an off-chain mixing approach with TEE proofs and on-chain dispute resolution only when needed.
...
The NeoVault service uses a service-specific Supabase repository for database operations.
...
services/neovault/
├── supabase/
│   ├── repository.go    # NeoVault-specific repository interface
│   └── models.go        # NeoVault data models (Request, MixOperation)
...
err := neovaultRepo.UpdateStatus(ctx, id, "delivered")
```
...
```bash
go test ./services/neovault/... -v -cover
```
