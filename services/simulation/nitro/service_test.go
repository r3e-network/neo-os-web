...
func TestNew(t *testing.T) {
	...
	t.Run("requires nitro", func(t *testing.T) {
		_, err := New(Config{})
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "nitro is required")
	})

	t.Run("invalid nitro type", func(t *testing.T) {
		_, err := New(Config{
			Nitro: "invalid",
		})
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "invalid nitro type")
	})
