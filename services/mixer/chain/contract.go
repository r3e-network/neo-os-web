...
// =============================================================================
// Mixer Contract Interface (v5.1 - Off-Chain First with On-Chain Dispute)
// =============================================================================
//
// Architecture: Off-Chain Mixing with On-Chain Dispute Resolution Only
// - User requests mix via API → Mixer service directly (NO on-chain)
...
// - Mixer delivers NetAmount to targets (fee deducted from delivery)
// - Fee collected from random pool account to master fee address
// - Normal path: User happy, ZERO on-chain link to service layer
// - Dispute path: User submits dispute → Mixer submits CompletionProof on-chain
//
...
func NewMixerContract(client *chain.Client, contractHash string, wallet *chain.Wallet) *MixerContract {
...
func (m *MixerContract) GetAdmin(ctx context.Context) (string, error) {
...
func (m *MixerContract) IsPaused(ctx context.Context) (bool, error) {
...
func (m *MixerContract) GetService(ctx context.Context, serviceID []byte) (*MixerServiceInfo, error) {
	...
	return parseMixerServiceInfo(result.Stack[0])
}

// GetDispute returns dispute information by request hash.
func (m *MixerContract) GetDispute(ctx context.Context, requestHash []byte) (*MixerDisputeInfo, error) {
	...
	return parseMixerDisputeInfo(result.Stack[0])
}

// IsDisputeResolved checks if a dispute has been resolved.
func (m *MixerContract) IsDisputeResolved(ctx context.Context, requestHash []byte) (bool, error) {
	params := []chain.ContractParam{chain.NewByteArrayParam(requestHash)}
...
func (m *MixerContract) ResolveDispute(ctx context.Context, requestHash, outputsHash, proofSignature []byte) (string, error) {
	if m.wallet == nil {
...
// MixerServiceInfo represents registered service information.
type MixerServiceInfo struct {
	ServiceID  []byte
	TeePubKey  []byte
	Bond       *big.Int
	Active     bool
...
// MixerDisputeInfo represents dispute information.
type MixerDisputeInfo struct {
	RequestHash   []byte
	User          string
	Amount        *big.Int
	ServiceID     []byte
...
// parseMixerServiceInfo parses service info from contract result.
func parseMixerServiceInfo(item chain.StackItem) (*MixerServiceInfo, error) {
	...
	if len(arr) < 5 {
		return nil, fmt.Errorf("invalid MixerServiceInfo: expected 5 items, got %d", len(arr))
	}
...
// parseMixerDisputeInfo parses dispute info from contract result.
func parseMixerDisputeInfo(item chain.StackItem) (*MixerDisputeInfo, error) {
	if item.Type != "Array" && item.Type != "Struct" {
		return nil, fmt.Errorf("expected Array or Struct, got %s", item.Type)
	}
	...
	if len(arr) < 8 {
		return nil, fmt.Errorf("invalid MixerDisputeInfo: expected 8 items, got %d", len(arr))
	}
