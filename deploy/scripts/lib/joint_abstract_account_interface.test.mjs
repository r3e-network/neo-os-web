...
test("joint AA validator accepts the exact registrar ABI and security gates", () => {
  const registryManifest = {
    ...
    permissions: [{
      contract: "*",
      methods: ["computePlatformAccountId", "registerPlatformAccount"],
    }],
  };
  const aaManifest = {
    abi: { methods: [
  ...
  const result = validateJointAbstractAccountInterface({
    ...
    aaSource: "Runtime.CallingScriptHash == registrar MinUpgradeDelayMs Platform registrar timelock not expired RegisterAccountCore( backupOwner false",
  });
