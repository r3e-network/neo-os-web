/**
 * Live Neo N3 testnet validation for MiniAppDiceGameV2.
 *
 * Funds the house, prepays reusable credit, completes two real
 * commit -> three-block beacon -> settle rounds, validates exact event identity
 * and the 5.70x payout rule, then proves reservation release, accounting deltas,
 * stats, and credit withdrawal. NEO_TESTNET_WIF is never printed.
 */
...
const live = createLiveRpc({
  network: "testnet",
  neon: pkg,
  label: "live_validate_dicegame_v2",
});
...
async function main() {
  ...
  const FUND = 300_000_000n;
  const DEPOSIT = 40_000_000n;
  ...
  console.log("\n[2] deposit 0.40 GAS reusable stake credit...");
  ...
  console.log(`\n[3] ${ROUNDS} commit/reveal rounds at 0.10 GAS...`);
  let wins = 0n;
  let losses = 0n;
  let payoutTotal = 0n;
  for (let round = 0; round < ROUNDS; round += 1) {
    ...
    const { log: commitLog } = await invoke(
      `commit${round + 1}`,
      "commit",
      [H(account.scriptHash), I(face), I(BET)],
    );
    ...
    const player = hash160Value(committed[1].value);
    const eventFace = BigInt(committed[2].value);
    const amount = BigInt(committed[3].value);
    const commitIndex = Number(committed[4].value);
    assert(player === account.scriptHash.toLowerCase(), "Committed player mismatch");
    assert(eventFace === BigInt(face), "Committed face mismatch");
    assert(amount === BET, "Committed amount mismatch");
    assert(
      integer(await read("reservedBankroll")) >= EXTRA,
    ...
    assert(hash160Value(settled[1].value) === player, "Settled player mismatch");
    assert(BigInt(settled[2].value) === eventFace, "Settled face mismatch");
    const rolled = BigInt(settled[3].value);
    const won = settled[4].value === true || settled[4].value === 1 || settled[4].value === "1";
    const payout = BigInt(settled[5].value);
    assert(rolled >= 1n && rolled <= 6n, "rolled face outside 1..6");
    if (won) {
      wins += 1n;
      payoutTotal += payout;
      assert(rolled === eventFace, "win roll does not match selected face");
      assert(payout === BET * 57n / 10n, `5.70x payout mismatch: ${payout}`);
    } else {
      losses += 1n;
      assert(rolled !== eventFace, "loss roll unexpectedly matches selected face");
      assert(payout === 0n, "loss paid a non-zero payout");
    }
    console.log(
      `    round${round + 1}: betId=${betId} face=${face} rolled=${rolled} ${won ? "WON 0.57" : "lost"}`,
    );
  }

  console.log("\n[4] accounting, stats, and reservation invariants...");
  const expectedBank = bank0 + FUND + losses * BET - wins * EXTRA;
  ...
  assert(bankEnd === expectedBank, "bankroll delta mismatch");
  assert(creditEnd === expectedCredit, "credit delta mismatch");
  assert(heldEnd === expectedHeld, "contract GAS delta mismatch");
  assert(pendingEnd === pending0, "pending count mismatch");
  assert(reservedEnd === 0n, "reserved bankroll not released");
  ...
  console.log(
    `\nPASS - Dice V2: ${wins} win(s), ${losses} loss(es), exact tx/event identity, 3-block reveal, 5.70x rule, reservation release, and credit recovery verified.`,
  );
}
