...
const controlMethods = new Set([
  ...
  "isAppRegistered", "isAppPaused", "update", "scheduleUpdate", "cancelUpdate",
  "setRegistry", "setPaused", "registerApp", "activateApp", "validateAndApplyDescriptor",
  "setDescriptor", "setAppPaused", "onNEP17Payment",
]);
...
export function evaluatePlatformVestingFramework({ manifest, surfaceSource, typesSource, indexSource }) {
  ...
  const wiring = {
    options_config: typesSource.includes("platformVesting?: FrameworkPlatformVestingConfig"),
    framework_surface: typesSource.includes("readonly platformVesting: FrameworkPlatformVestingSurface"),
    composition_root: indexSource.includes("createPlatformVestingSurface") &&
      indexSource.includes("get platformVesting()"),
    scoped_write_guard: surfaceSource.includes("WRITE_PLATFORM_VESTING"),
  };
  return {
...
export function buildPlatformVestingFrameworkReport({ now = () => new Date() } = {}) {
  ...
  return {
    ...
    boundary: "PlatformVesting is source/build/test accepted and exposed through a guarded framework surface, but it has no retained deployment record or live binding. NeoPay remains a separate live reference until an approved deployment, registry binding, funded lifecycle, and exact read-back are proven.",
    chain_writes_performed: false,
  };
}

export function renderPlatformVestingFrameworkMarkdown(report) {
  return `${[
...
export function writePlatformVestingFrameworkReport({ check = false } = {}) {
  const report = buildPlatformVestingFrameworkReport();
  const jsonPath = path.join(repoRoot, "docs/reports/platform-vesting-framework-interface-latest.json");
  const markdownPath = path.join(repoRoot, "docs/reports/platform-vesting-framework-interface-latest.md");
  const json = `${JSON.stringify(report, null, 2)}\n`;
  const markdown = renderPlatformVestingFrameworkMarkdown(report);
  if (check) {
    const existingJson = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
    const currentJson = JSON.parse(json);
    delete existingJson.generated_at_utc;
    delete currentJson.generated_at_utc;
    if (JSON.stringify(existingJson) !== JSON.stringify(currentJson)) throw new Error("PlatformVesting framework JSON is stale");
    ...
    if (existingMarkdown !== currentMarkdown) throw new Error("PlatformVesting framework Markdown is stale");
  } else {
    fs.writeFileSync(jsonPath, json);
    fs.writeFileSync(markdownPath, markdown);
  }
  if (!report.passed) throw new Error("PlatformVesting framework interface audit failed");
  return report;
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const report = writePlatformVestingFrameworkReport({ check: process.argv.includes("--check") });
  console.log(`PlatformVesting framework interface: ${report.tenant_abi_method_count}/${report.tenant_abi_method_count} PASS`);
}
