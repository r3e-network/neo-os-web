-- =============================================================================
-- OneGate Vault failed payout retry request id rotation
-- =============================================================================
-- A failed payout keeps the QR key bound to the original wallet and amount so
-- the key is still one-use. Retrying must not reuse the old txproxy request_id,
-- because txproxy correctly treats it as replay. Rotate only the request_id and
-- status; do not re-randomize the amount or decrement the pool again.
