      ...
      <NeoCard variant="erobo-neo">
        <div class="vote-form">
          <NeoCard v-if="selectedCandidate" variant="erobo-neo" flat class="selected-candidate-card">
            ...
            <div class="candidate-badge">
              <div class="logo-name-row">
                ...
                />
                <span class="candidate-name">{{ selectedCandidate.name || selectedCandidate.address }}</span>
              </div>
              <span v-if="selectedCandidate.description" class="candidate-desc">
                {{ selectedCandidate.description }}
              </span>
              <div class="details-grid">
                <div class="detail-item">
                  <span class="detail-label">{{ t("publicKey") }}</span>
                  <span class="detail-value mono">{{ selectedCandidate.publicKey }}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">{{ t("address") }}</span>
                  <span class="detail-value mono">{{ selectedCandidate.address }}</span>
                </div>
              </div>
            </div>
          </NeoCard>
          ...
          <NeoCard v-else variant="warning" flat class="no-candidate-card">
            <span class="warning-text text-center">{{ t("selectCandidateFirst") }}</span>
          </NeoCard>
          ...
          <div v-if="!address" class="connect-hint">
            <span class="hint-text">{{ t("connectWallet") }}</span>
          </div>
        </div>
      </NeoCard>
    </template>
