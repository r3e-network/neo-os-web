<template>
  <NeoCard variant="erobo-neo" class="garden-card-glass">
    <div class="garden-container-glass">
      <div class="garden-grid-glass">
        <div
          v-for="plot in plots"
          :key="plot.id"
          class="plot-glass"
        ...
        >
          <div v-if="plot.plant" class="plant-box-glass">
            <span class="plant-icon-glass" :class="{ ready: plot.plant.growth >= 100 }">
              {{ plot.plant.icon }}
            </span>
            <div v-if="plot.plant.growth >= 100" class="ready-sticker-glass">{{ readyLabel }}</div>
          </div>
          <span v-else class="empty-icon-glass">🕳️</span>
          <div v-if="plot.plant" class="growth-label-glass">
            <span class="growth-text-glass">{{ Math.floor(plot.plant.growth) }}%</span>
          </div>
        </div>
      </div>
    </div>
