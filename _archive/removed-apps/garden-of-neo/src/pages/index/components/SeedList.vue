  ...
  <NeoCard variant="erobo" class="mb-4">
    <ItemList :items="seeds" item-key="id">
      <template #item="{ item: seed }">
        ...
        >
          <div class="seed-icon-wrapper-glass">
            <span class="seed-icon">{{ seed.icon }}</span>
          </div>
          <div class="seed-info">
            <span class="seed-name-glass">{{ seed.name }}</span>
            <span class="seed-time-glass">⏱ {{ seed.growTime }}{{ hoursLabel }}</span>
          </div>
          <div class="seed-price-tag-glass">
            <span class="seed-price-glass">{{ seed.price }}</span>
            <span class="seed-currency-glass">GAS</span>
          </div>
        </div>
      </template>
    </ItemList>
