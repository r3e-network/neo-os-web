<template>
  ...
  >
    <div class="entrant-inner">
      <!-- Rank -->
      <div class="rank-glass" :class="'rank-' + rank">
        <span>#{{ rank }}</span>
      </div>

      <!-- Avatar -->
      <div class="avatar-glass">
        <span class="avatar-text-glass">{{ entrant.name.charAt(0) }}</span>
      </div>

      <!-- Info -->
      <div class="entrant-info">
        <span class="entrant-name-glass">{{ entrant.name }}</span>
        <div class="score-row">
          <span class="fire-glass">&#x1F525;</span>
          <span class="score-glass">{{ formattedScore }} GAS</span>
        </div>
      </div>
    ...
    <div class="progress-track-glass">
      ...
      >
        <div class="progress-glow" v-if="rank === 1"></div>
      </div>
    </div>
  </NeoCard>
</template>
