<template>
  <div class="grid-layout px-1">
    ...
    >
      ...
      <div class="shine-effect pointer-events-none absolute inset-0 z-0" />

      <div class="game-header relative z-10 mb-1 text-center">
        <span class="game-title text-sm font-black tracking-tighter text-white uppercase decoration-2 opacity-80">
          {{ game.name }}
        </span>
      ...
      <div class="game-visual relative my-3 flex h-32 items-center justify-center">
        ...
        >
          ...
          <span class="text-[9px] font-black tracking-[0.2em] text-white/40 uppercase">PREMIUM</span>
        </div>
      </div>

      <div class="game-stats relative z-10 mt-auto mb-5 text-center">
        <span class="mb-1 block text-[10px] font-bold tracking-[0.1em] text-white uppercase opacity-70"> >JACKPOT</span>
        <span class="glow-text block text-3xl leading-none font-black text-white italic">
          {{ game.maxJackpotDisplay.split(" ")[0] }}
          <span class="ml-1 text-xs italic opacity-70">GAS</span>
        </span>
      </div>
      ...
      >
        <div class="flex items-center gap-2">
          <span class="text-xs font-black uppercase italic">{{ t("buyTicket") }}</span>
          <AppIcon name="arrow-right" :size="14" />
        </div>
      </NeoButton>

      <div class="mt-3 flex min-h-[32px] items-center justify-center">
        <span class="px-2 text-center text-[10px] leading-tight font-medium text-white opacity-60">
          {{ game.description }}
        </span>
      </div>
    </div>
