  ...
  <ActionModal :visible="isOpen" :title="typeInfo.name" :closeable="!revealing" size="md" @close="handleClose">
    <div class="scratch-card-area">
      ...
      <div class="result-layer">
        <div v-if="revealing" class="revealing-spinner">
          <AppIcon name="loader" size="32" class="animate-spin" />
          <span class="text-gold mt-2">{{ t("revealing") }}</span>
        </div>

        <div v-else-if="result" class="result-content" aria-live="assertive">
          <template v-if="result.isWinner">
            <span class="win-icon" aria-hidden="true">🎉</span>
            <span class="win-amount">{{ formatNum(result.prize) }} GAS</span>
            <span class="win-label">{{ t("youWon") }}</span>
          </template>
          <template v-else>
            <span class="lose-icon" aria-hidden="true">😢</span>
            <span class="lose-label">{{ t("betterLuck") }}</span>
          </template>
        </div>
      </div>
      ...
      <div
        ...
        @click="scratch"
      >
        <span class="scratch-hint">{{ t("clickToScratch") }}</span>
        <span class="scratch-price">{{ typeInfo.priceDisplay }}</span>
      </div>
    </div>
