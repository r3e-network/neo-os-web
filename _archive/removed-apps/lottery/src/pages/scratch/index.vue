  ...
  <ResponsiveLayout :desktop-breakpoint="1024" :title="t('scratchTitle')" class="theme-chinese-lucky">
    <div class="scratch-gallery">
      ...
      <div class="gallery-header">
        <span class="title">{{ t("scratchSelectTicket") }}</span>
        <span class="subtitle">{{ t("scratchSubtitle") }}</span>
      </div>
      ...
      <div class="lottery-grid">
        ...
        >
          <div class="card-banner">
            ...
            <span class="lottery-price">{{ lottery.priceDisplay }}</span>
          </div>
          <div class="card-info">
            <span class="max-prize">{{ t("maxPrize") }} {{ lottery.maxJackpotDisplay }}</span>
            <span class="description">{{ lottery.description }}</span>
          </div>
        </div>
      </div>

      <!-- My Tickets Section -->
      <div class="my-tickets" v-if="unrevealedTickets.length > 0">
        <span class="section-title">{{ t("scratchMyTickets", { count: unrevealedTickets.length }) }}</span>
        <scroll-view scroll-x class="tickets-scroll">
          ...
          >
            ...
            <span class="ticket-action">{{ t("scratchPlayAction") }}</span>
          </div>
        </scroll-view>
      </div>
    </div>
