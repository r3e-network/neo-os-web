<template>
  <ResponsiveLayout :desktop-breakpoint="1024" :title="t('scratchPlayTitle')" class="theme-chinese-lucky">
    <div class="scratch-play">
      <!-- Lottery Info -->
      <div class="lottery-info" v-if="currentLottery">
        <span class="lottery-name">{{ currentLottery.name }}</span>
        <span class="lottery-price">{{ currentLottery.priceDisplay }}</span>
      </div>
      ...
      <div class="scratch-area">
        <canvas
          ...
          @touchend="onTouchEnd"
        />
        <div class="prize-layer" :class="{ revealed: isRevealed }">
          <span class="prize-amount" v-if="prize > 0">
            {{ t("scratchWinInline", { amount: formatPrize(prize) }) }}
          </span>
          <span class="no-prize" v-else>
            {{ t("scratchNoPrize") }}
          </span>
        </div>
      </div>
      ...
      <!-- Status Message -->
      <div class="status-message" v-if="status" :class="`status-message--${status.type}`">
        <span>{{ status.msg }}</span>
      </div>

      <!-- Prize Tiers -->
      <div class="prize-tiers">
        <span class="tiers-title">{{ t("prizeTiers") }}</span>
        <div class="tier-list">
          <div v-for="tier in PRIZE_TIERS" :key="tier.tier" class="tier-item">
            <span class="tier-label">{{ getPrizeTierLabel(tier.tier) }}</span>
            <span class="tier-odds">{{ tier.odds }}%</span>
