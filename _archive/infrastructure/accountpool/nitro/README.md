# AccountPool Nitro Service

TEE-secured HD-derived account pool management service running inside NitroRun enclave.

## Overview

The AccountPool Nitro service manages a pool of Neo N3 accounts derived from a master key:
...
- `masterKey`: From NitroRun injection (stable across upgrades)
...
- Master key never leaves NitroRun TEE
...
In strict identity mode (strict production/NitroRun TLS), caller identity is
derived from verified mTLS peer identity; inter-service calls should use the
NitroRun-provided mTLS HTTP client.
