...
func runStatus(rpcURL, configFile string) {
	...
	if err != nil {
		...
		if blockCount, blockErr := deployer.GetBlockCount(); blockErr != nil {
			log.Printf("Warning: Could not fetch block count: %v", blockErr)
		} else {
			log.Printf("Block height: %d", blockCount)
		}
		if gasBalance, balanceErr := deployer.GetGASBalanceFloat(); balanceErr != nil {
			log.Printf("Warning: Could not fetch deployer GAS balance: %v", balanceErr)
		} else {
...
func runDeploy(rpcURL, configFile, buildDir, contractName string, dryRun bool) {
	...
	registry := chain.NewContractRegistry("testnet", filepath.Dir(configFile))
	if err := registry.LoadFromFile(configFile); err != nil {
		log.Printf("Warning: Could not load registry from %s: %v", configFile, err)
	}
...
func runExport(configFile, format string) {
	...
	case "json":
		...
		if err != nil {
			log.Fatalf("failed to encode addresses as json: %v", err)
		}
