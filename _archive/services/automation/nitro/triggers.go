...
func (s *Service) dispatchAction(ctx context.Context, actionRaw json.RawMessage) error {
	...
	case "webhook":
		...
		if strict && useMeshClient {
			if m := s.Nitro(); m == nil || m.TLSConfig() == nil {
				return fmt.Errorf("mesh webhook requires Nitro mTLS in strict identity mode")
			}
		}
		...
		// If this is an internal (mesh) URL and Nitro mTLS is available, enforce
		// HTTPS so peer identity can be verified.
		if m := s.Nitro(); m != nil && m.TLSConfig() != nil && useMeshClient {
