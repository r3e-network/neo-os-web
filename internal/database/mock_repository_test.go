...
func TestMockRepository_Reset(t *testing.T) {
	...
	// Add some data
	repo.CreateUser(ctx, &User{Address: "test"})
	repo.CreateSecret(ctx, &Secret{UserID: "user-1", Name: "secret-1"})
	...
	secrets, _ := repo.GetSecrets(ctx, "user-1")
	if len(secrets) != 0 {
		t.Error("Reset() should clear all data")
	}
}
...
func TestMockRepository_SecretOperations(t *testing.T) {
	repo := NewMockRepository()
	ctx := context.Background()
	...
	if err := repo.CreateSecret(ctx, secret); err != nil {
		t.Fatalf("CreateSecret() error = %v", err)
	}

	// Test GetSecrets
	list, err := repo.GetSecrets(ctx, "user-123")
	if err != nil {
		t.Fatalf("GetSecrets() error = %v", err)
	}
	if len(list) != 1 {
		t.Errorf("GetSecrets() len = %v, want 1", len(list))
	}
	if list[0].Name != "API_KEY" {
		t.Errorf("GetSecrets() Name = %v, want API_KEY", list[0].Name)
	}
}
